...
Public Class Part_List
    ...
    Dim SQL_str As String = "SELECT  Id, Nam FROM  Define_PartKala WHERE Activ='False'"
    ...
    Private Sub GetUser()
        Try
            ...
            End If
            '''''''''''''''''''''''''''
            Dta = New SqlDataAdapter(SQL_str, DataSource)
            Dta.Fill(Ds, "Define_PartKala")
            DGV.AutoGenerateColumns = False
            dv = Ds.Tables("Define_PartKala").DefaultView
            DGV.DataSource = dv
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Part_List", "GetUser")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub RefreshBank()
        Try
            ...
            TxtSearch.Text = ""
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Part_List", "RefreshBank")
        End Try
    End Sub
    ...
    Private Sub BtnNewP_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnNewP.Click
        Try
            Fnew = True
            Using FrmAnbar As New DefinePart
                FrmAnbar.ShowDialog()
            End Using
            Fnew = False
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Part_List", "BtnNewP_Click")
        End Try
    End Sub
