...
Public Class SendAllSMSChk
    ...
    Private Sub Filldata()
        Try
            ...
            DGV1.DataSource = dt
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "SendAllSMSChk", "Filldata")
            Me.Close()
        End Try
    End Sub
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Try
            ...
            For i As Integer = 0 To DGV1.RowCount - 1
                ...
                Else
                    ...
                    lSendReference = ActiveSMS.axKylixSMS.SendSMS(DGV1.Item("cln_tell", i).Value, GetMessage(DGV1.Item("cln_name", i).Value, DGV1.Item("cln_Mon", i).Value, DGV1.Item("cln_State", i).Value, SUBDAY(DGV1.Item("Cln_Delay", i).Value, GetDate()), DGV1.Item("Cln_StateChk", i).Value))
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "SendAllSMSChk", "Button1_Click")
            Button1.Enabled = True
            ChkInvalid.Enabled = True
            DGV1.AllowUserToDeleteRows = True
            Button2.Enabled = False
    ...
    Private Function GetMessage(ByVal nam As String, ByVal Mon As Double, ByVal Numchk As String, ByVal day As Long, ByVal state As String) As String
        Dim str As String = ""
        If state = "در جریان وصول" Or state = "برات" Then
            If day = 0 Then
                str = " سر رسید چک " & Numchk & "به مبلغ" & IIf(Mon <> 0, FormatNumber(Mon, 0), 0) & "به اتمام رسیده است " & " از طرف:" & namcompany
            ElseIf day > 0 Then
                str = day & " روز از سر رسید چک " & Numchk & "به مبلغ" & IIf(Mon <> 0, FormatNumber(Mon, 0), 0) & "مانده است " & " از طرف:" & namcompany
            ElseIf day < 0 Then
                str = day * (-1) & " روز از سر رسید چک " & Numchk & "به مبلغ" & IIf(Mon <> 0, FormatNumber(Mon, 0), 0) & "گذشته است " & " از طرف:" & namcompany
            End If
        ElseIf state = "وصول شده" Then
            str = "  چک " & Numchk & "به مبلغ" & IIf(Mon <> 0, FormatNumber(Mon, 0), 0) & "وصول شد " & " از طرف:" & namcompany
        ElseIf state = "برگشتی" Then
            str = "  چک " & Numchk & "به مبلغ" & IIf(Mon <> 0, FormatNumber(Mon, 0), 0) & "برگشت داده شد " & " از طرف:" & namcompany
        End If
        Return str
    End Function
    ...
    Private Sub DelInvalidNumber()
        Try
            ...
            Next
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "SendAllSMSChk", "DelInvalidNumber")
        End Try
    End Sub
