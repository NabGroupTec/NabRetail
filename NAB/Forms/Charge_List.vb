...
Public Class Charge_List
    ...
    Private Sub GetCharge()
        Try
            ...
            End If
            '''''''''''''''''''''''''''
            Dta = New SqlDataAdapter("SELECT  Define_Charge.nam, Define_Charge.ID, Define_Charge.IdCodeTwo, Define_Charge.IdOne, Define_OneCharge.NamOne FROM  Define_Charge INNER JOIN  Define_OneCharge ON Define_Charge.IdOne = Define_OneCharge.Id", DataSource)
            Dta.Fill(Ds, "Define_Charge")
            CmbOstan.Items.Clear()
            For i As Integer = 0 To Ds.Tables("Define_Charge").Rows.Count - 1
                If CmbOstan.FindStringExact(Ds.Tables("Define_Charge").Rows(i).Item("NamOne")) = -1 Then
                    CmbOstan.Items.Add(Ds.Tables("Define_Charge").Rows(i).Item("NamOne"))
                End If
            Next
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Charge_List", "GetCharge")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub RefreshBank()
        Try
            ...
            Next
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Charge_List", "RefreshBank")
        End Try
    End Sub
                ...
                If RdoAval.Checked = True Then
                    ...
                    ElseIf RdoCode.Checked = True Then
                        If TxtCode.Text.Contains("+") Then
                            Dim x() As String
                            Dim Con As String = ""
                            x = TxtCode.Text.Split("+")

                            For i As Integer = 0 To x.Length - 1
                                If Not String.IsNullOrEmpty(x(i)) Then Con &= "IdCodeTwo Like '" & x(i) & "%' OR "
                            Next

                            If String.IsNullOrEmpty(Con) Then
                ...
                If RdoAval.Checked = True Then
                    ...
                    ElseIf RdoCode.Checked = True Then
                        If TxtCode.Text.Contains("+") Then
                            ...
                            Else
                                Con = "Nam Like '" & TxtSearch.Text.Trim & "%'  AND " & Con.Substring(0, Con.Length - 3)
                                dv.RowFilter = Con
                            End If
            ...
            ElseIf e.KeyCode = Keys.Enter Then
                Try
                    If DGV.RowCount > 0 Then
                        ...
                        Tmp_TwoGroup = If(DGV.Item("Cln_City", DGV.CurrentRow.Index).Value.ToString Is DBNull.Value, "", DGV.Item("Cln_City", DGV.CurrentRow.Index).Value.ToString)
                        Me.Close()
                    End If
                Catch ex As Exception
                    IdKala = 0
                    ...
                    If DGV.RowCount > 0 Then
                        ...
                        For i As Integer = 0 To DGV.RowCount - 1
                            If DGV.Item("Cln_Select", i).Value = True Then
                                ...
                                AllSelectKala(AllSelectKala.Length - 1).IdKala = If(DGV.Item("Cln_City", i).Value.ToString Is DBNull.Value, "", DGV.Item("Cln_City", i).Value.ToString)
                ...
                If DGV.RowCount > 0 Then
                    ...
                    For i As Integer = 0 To DGV.RowCount - 1
                        If DGV.Item("Cln_Select", i).Value = True Then
                            ...
                            AllSelectKala(AllSelectKala.Length - 1).Namekala = DGV.Item("Cln_Nam", i).Value
                            AllSelectKala(AllSelectKala.Length - 1).OneGroup = DGV.Item("Cln_Ostan", i).Value
                            AllSelectKala(AllSelectKala.Length - 1).TwoGroup = If(DGV.Item("Cln_City", i).Value.ToString Is DBNull.Value, "", DGV.Item("Cln_City", i).Value.ToString)
    ...
    Private Sub BtnNewP_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnNewP.Click
        Try
            Fnew = True
            Using FrmPeople As New DefineCharge
                FrmPeople.ShowDialog()
            End Using
            Fnew = False
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Charge_List", "BtnNewP_Click")
        End Try
    End Sub
