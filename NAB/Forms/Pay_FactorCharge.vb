...
Public Class Pay_FactorCharge
    ...
    Private Sub BtnHavaleh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnHavaleh.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "BtnHavaleh_Click")
        End Try
    End Sub
    Private Sub GetKey(ByVal e As KeyEventArgs)
        ...
        ElseIf e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "Charge_Buy.htm")
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "BtnSave_Click")
        End Try
    End Sub
    Private Function Savepay() As Boolean
        ...
        Try
            ...
            If CDbl(TxtCash.Text) > 0 Then
                Using Cmd As New SqlCommand("INSERT Moein_Box (D_date,disc,mon,T,IdBox,IdUser) VALUES(@D_date,@disc,@mon,@T,@IdBox,@IdUser);SELECT @@IDENTITY", ConectionBank)
                    ...
                    Cmd.Parameters.AddWithValue("@Disc", SqlDbType.NVarChar).Value = "پرداخت هزینه ف.خرید" & LBuyFac.Text & " بابت سند  " & LIdFac.Text & If(String.IsNullOrEmpty(TxtDisc_Cash.Text), "", " - " & TxtDisc_Cash.Text.Trim)
            ...
            If CDbl(Txtbank.Text) > 0 Then
                Using Cmd As New SqlCommand("INSERT Moein_Bank (D_date,disc,mon,T,IDBank,IdUser) VALUES(@D_date,@disc,@mon,@T,@IDBank,@IdUser);SELECT @@IDENTITY", ConectionBank)
                    Cmd.Parameters.AddWithValue("@D_date", SqlDbType.NVarChar).Value = TxtDate.ThisText
                    Cmd.Parameters.AddWithValue("@Disc", SqlDbType.NVarChar).Value = "پرداخت هزینه ف.خرید" & LBuyFac.Text & " بابت سند  " & LIdFac.Text & If(String.IsNullOrEmpty(TxtDiscbank.Text), "", " - " & TxtDiscbank.Text.Trim)
            ...
            If CDbl(TxtLend.Text) > 0 Then
                Using Cmd As New SqlCommand("INSERT Moein_People (D_date,disc,mon,T,IdPeople,IdUser,Type,Number_Type,Type_Sanad) VALUES(@D_date,@disc,@mon,@T,@IdPeople,@IdUser,@Type,@Number_Type,@Type_Sanad);SELECT @@IDENTITY", ConectionBank)
                    Cmd.Parameters.AddWithValue("@D_date", SqlDbType.NVarChar).Value = TxtDate.ThisText
                    Cmd.Parameters.AddWithValue("@Disc", SqlDbType.NVarChar).Value = "پرداخت هزینه ف.خرید" & LBuyFac.Text & " بابت سند  " & LIdFac.Text & If(String.IsNullOrEmpty(TxtDiscLend.Text), "", " - " & TxtDiscLend.Text.Trim)
            ...
            End If
            '''''''''''''''ListFacotr
            Using Cmd As New SqlCommand("Update ListFactorCharge SET Activ=@Activ,EditFlag=@EditFlag,D_date=@D_date,IdName=@IdName,Discount=@Discount,Cash=@Cash,MonHavaleh=@MonHavaleh,IdBankHavaleh=@IdBankHavaleh,DiscHavaleh=@DiscHavaleh,MonSellChk=@MonSellChk,MonPayChk=@MonPayChk,Lend=@Lend,IdSanadLend=@IdSanadLend,DiscDisC=@DiscDisC,DiscCash=@DiscCash,DiscSellChk=@DiscSellChk,DiscChk=@DiscChk,IdBox=@IdBox,IdBoxMoein=@IdBoxMoein,IdBanKMoein=@IdBanKMoein,DiscLend=@DiscLend WHERE Id=@Id", ConectionBank)
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "Savepay")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Function Editpay() As Boolean
        ...
        Try
            ...
            Else
                If Not String.IsNullOrEmpty(TmpId) Then
                    Using Cmd As New SqlCommand("UPDATE ListFactorCharge SET IdBoxMoein=@IdBoxMoein,IdBox=@IdBox WHERE Id=@IdSanad;DELETE FROM  Moein_Box WHERE Id=@ID", ConectionBank)
            ...
            Else
                If Not String.IsNullOrEmpty(TmpId) Then
                    Using Cmd As New SqlCommand("UPDATE ListFactorCharge SET IdBankMoein=@IdBankMoein WHERE Id=@IdSanad;DELETE FROM  Moein_Bank WHERE Id=@ID", ConectionBank)
            ...
            Else
                If Not String.IsNullOrEmpty(TmpId) Then
                    Using Cmd As New SqlCommand("UPDATE ListFactorCharge SET IdSanadLend=@IdSanadLend WHERE Id=@IdSanad;DELETE FROM  Moein_People WHERE Id=@ID", ConectionBank)
            ...
            Using Cmd As New SqlCommand("Update ListFactorCharge SET Sanad=@Sanad,IdFactor=@IdFactor,Activ=@Activ,EditFlag=@EditFlag,D_date=@D_date,IdName=@IdName,Discount=@Discount,Cash=@Cash,MonHavaleh=@MonHavaleh,IdBankHavaleh=@IdBankHavaleh,DiscHavaleh=@DiscHavaleh,MonSellChk=@MonSellChk,MonPayChk=@MonPayChk,Lend=@Lend,IdSanadLend=@IdSanadLend,DiscDisC=@DiscDisC,DiscCash=@DiscCash,DiscSellChk=@DiscSellChk,DiscChk=@DiscChk,IdBox=@IdBox,IdBoxMoein=@IdBoxMoein,IdBanKMoein=@IdBanKMoein,DiscLend=@DiscLend,Disc=@Disc WHERE Id=@Id", ConectionBank)
                ...
                Cmd.Parameters.AddWithValue("@Id", SqlDbType.BigInt).Value = CLng(LIdFac.Text)
                Cmd.Parameters.AddWithValue("@IdFactor", SqlDbType.BigInt).Value = CLng(LFactor.Text)
                Cmd.ExecuteNonQuery()
            End Using

            ''''''''''''''''KalaCharge
            Using Cmd As New SqlCommand("DELETE FROM KalaFactorCharge WHERE IdSanad=@IdSanad", ConectionBank)
                Cmd.Parameters.AddWithValue("@IdSanad", SqlDbType.BigInt).Value = CLng(LIdFac.Text)
                Cmd.ExecuteNonQuery()
            End Using
            TmpId = "INSERT INTO KalaFactorCharge (IdSanad,IdCharge,Mon,CDisc) VALUES(@IdSanad,@IdCharge,@Mon,@CDisc)"
        ...
        Catch ex As Exception
            ...
            sqltransaction.Dispose()
            If Err.Number = 5 Then
                MessageBox.Show("اطلاعات پرداختی فاکتور قابل ویرایش شدن نیست", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "Editpay")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Sub BtnCancle_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnCancle.Click
        Try
            ...
            Me.Close()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "BtnCancle_Click")
        End Try
    End Sub
    ...
    Private Sub BtnSodor_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSodor.Click
        Try
            Using FrmSodor As New Sodor_Check
                FrmSodor.LName.Text = "هزینه فاکتور خرید"
                FrmSodor.LIdname.Text = "-17"
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "BtnSodor_Click")
        End Try
    End Sub
    ...
    Private Sub GetBoxList()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "GetBoxList")
        End Try
    End Sub
    ...
    Private Sub GetInfo()
        Try
            ...
            '////////////////////////////////////////
            Using Cmd As New SqlCommand("SELECT  D_date, IdName, Discount, Cash, MonHavaleh, IdBankHavaleh, DiscHavaleh, MonSellChk, MonPayChk, Lend, IdSanadLend, DiscDisC, DiscCash,DiscSellChk, DiscChk, IdBox, IdBoxMoein, IdBanKMoein, DiscLend FROM  ListFactorCharge WHERE ID=" & CLng(LIdFac.Text), ConectionBank)
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "GetInfo")
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Me.Close()
        End Try
    ...
    Private Sub BtnFroshChk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnFroshChk.Click
        Try
            Using FrmFrosh As New Frosh_Check
                FrmFrosh.LName.Text = "هزینه فاکتور خرید"
                FrmFrosh.LIdname.Text = "-17"
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "BtnFroshChk_Click")
        End Try
    End Sub
    ...
    Private Sub BtnDiscDisC_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDiscDisC.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "BtnDiscDisC_Click")
        End Try
    End Sub
    ...
    Private Sub BtnDiscCash_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDiscCash.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "BtnDiscCash_Click")
        End Try
    End Sub
    ...
    Private Sub BtnDiscHavaleh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDiscHavaleh.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "BtnDiscHavaleh_Click")
        End Try
    End Sub
    ...
    Private Sub BtnDisc_SellChk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDisc_SellChk.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "BtnDisc_SellChk_Click")
        End Try
    End Sub
    ...
    Private Sub BtnDiscChk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDiscChk.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "BtnDiscChk_Click")
        End Try
    End Sub
    ...
    Private Sub BtnBox_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnBox.Click
        Try
            ...
            Me.GetBoxList()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "BtnBox_Click")
        End Try
    End Sub
    ...
    Private Sub BtnDiscLend_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDiscLend.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Pay_FactorCharge", "BtnDiscLend_Click")
        End Try
    End Sub
