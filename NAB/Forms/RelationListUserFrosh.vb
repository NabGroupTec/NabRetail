...
Public Class RelationListUserFrosh

    Dim ds As New DataSet
    Dim str As String = "SELECT  ListUser.IdUser, Define_User.Nam FROM ListUser INNER JOIN Define_User ON ListUser.IdUser = Define_User.Id"
    ...
    Private Sub Fill_KDgv(ByVal Idvisit As Long)
        Try
            DGV1.DataSource = Nothing
            dsK.Clear()
            strK = "SELECT REPLACE(ListkalaU.Kala,'.','/') As Kala , ListkalaU.Mon,ListkalaU.IdKala,ListkalaU.Id,Define_Kala.Nam FROM ListkalaU INNER JOIN Define_Kala ON ListkalaU.Idkala = Define_Kala.Id WHERE IdLinkUser =" & Idvisit
            If Not dtaK Is Nothing Then
                dtaK.Dispose()
            End If
            '''''''''''''''''''''''''''
            dtaK = New SqlDataAdapter(strK, DataSource)
            dtaK.Fill(dsK, "ListkalaU")
            DGV1.AutoGenerateColumns = False
            bsK.DataSource = dsK
            bsK.DataMember = "ListkalaU"
            DGV1.DataSource = bsK
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh", "fill_Kdgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Fill_DDgv(ByVal Idvisit As Long)
        Try
            DGV2.DataSource = Nothing
            dsD.Clear()
            strD = "SELECT REPLACE(ListDarsadU.Frosh,'.','/') As Frosh, REPLACE(ListDarsadU.Darsad,'.','/') AS Darsad, Define_Group_P.nam FROM ListDarsadU INNER JOIN Define_Group_P ON ListDarsadU.IdGroup = Define_Group_P.Id WHERE IdlinkKala =" & Idvisit
            If Not dtaD Is Nothing Then
                dtaD.Dispose()
            End If
            '''''''''''''''''''''''''''
            dtaD = New SqlDataAdapter(strD, DataSource)
            dtaD.Fill(dsD, "ListDarsadU")
            DGV2.AutoGenerateColumns = False
            bsD.DataSource = dsD
            bsD.DataMember = "ListDarsadU"
            DGV2.DataSource = bsD
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh", "fill_Ddgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Fill_ODgv(ByVal Idvisit As Long)
        Try
            DGV4.DataSource = Nothing
            dsO.Clear()
            strO = "SELECT  REPLACE(ListOrderDarsadU.Darsad,'.','/') As Darsad, Define_Group_P.nam FROM ListOrderDarsadU INNER JOIN Define_Group_P ON ListOrderDarsadU.IdGroup = Define_Group_P.Id WHERE IdLinkUser =" & Idvisit
            If Not dtaO Is Nothing Then
                dtaO.Dispose()
            End If
            '''''''''''''''''''''''''''
            dtaO = New SqlDataAdapter(strO, DataSource)
            dtaO.Fill(dsO, "ListOrderDarsadU")
            DGV4.AutoGenerateColumns = False
            bsO.DataSource = dsO
            bsO.DataMember = "ListOrderDarsadU"
            DGV4.DataSource = bsO
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh", "Fill_ODgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub RelationListUserFrosh_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
    End Sub

    Private Sub RelationListUserFrosh_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Me.GetKey(e)
    End Sub

    Private Sub RelationListUserFrosh_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub SetButton()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh", "SetButton")
            Me.Close()
        End Try
    End Sub
    Private Sub Fill_Dgv()
        Try
            ...
            dta.Fill(ds, "ListUser")
            DGV3.AutoGenerateColumns = False
            bs.DataSource = ds
            bs.DataMember = "ListUser"
            DGV3.DataSource = bs
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh", "fill_dgv")
            Me.Close()
        End Try
    End Sub
    Private Sub RefreshBank()
        Try
            ...
            dta.Fill(ds, "ListUser")
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh", "RefreshBank")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub cmdadd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdadd.Click
        Try
            Using FrmAdd As New RelationUserFrosh
                FrmAdd.EDIT = 0
                FrmAdd.ShowDialog()
            End Using
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh", "cmdadd_Click")
            Me.RefreshBank()
        End Try
    End Sub
    ...
    Private Sub cmddel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmddel.Click
        Try
            ...
            If str = 6 Then
                ...
                Me.RefreshBank()
                TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "رابطه کاربر و فروش هدف-کالا", "حذف", "حذف فروش هدف کاربر :" & nam, "")
            End If
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh", "cmddel_Click")
            Me.RefreshBank()
        End Try
    End Sub
    Private Function DeleteUser(ByVal Id As Long) As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("DELETE FROM ListUser WHERE IdUser=@IdUser", ConectionBank)
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh", "DeleteUser")
            End If
            Me.RefreshBank()
            Return False
        End Try
