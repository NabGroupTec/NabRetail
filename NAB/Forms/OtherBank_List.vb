...
Public Class OtherBank_List
    ...
    Private Sub GetBank()
        Try
            ...
            End If
            '''''''''''''''''''''''''''
            Dta = New SqlDataAdapter("SELECT ID, nam FROM Define_OtherBank", DataSource)
            Dta.Fill(Ds, "Define_OtherBank")
            DGV.AutoGenerateColumns = False
            dv = Ds.Tables("Define_OtherBank").DefaultView
            DGV.DataSource = dv
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "OtherBank_List", "Getbank")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub RefreshBank()
        Try
            ...
            TxtSearch.Text = ""
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "OtherBank_List", "RefreshBank")
        End Try
    End Sub
    ...
    Private Sub BtnNewP_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnNewP.Click
        Try
            Fnew = True
            Using FrmAnbar As New DefineOtherBank
                FrmAnbar.ShowDialog()
            End Using
            Fnew = False
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "OtherBank_List", "BtnNewP_Click")
        End Try
    End Sub
