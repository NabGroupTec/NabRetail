...
Public Class Daramad_List
    ...
    Private Sub GetDaramad()
        Try
            ...
            End If
            '''''''''''''''''''''''''''
            Dta = New SqlDataAdapter("SELECT  Define_Daramad .nam, Define_Daramad .ID, Define_Daramad .IdCodeTwo, Define_Daramad .IdOne, Define_OneDaramad .NamOne FROM  Define_Daramad  INNER JOIN  Define_OneDaramad  ON Define_Daramad .IdOne = Define_OneDaramad .Id", DataSource)
            Dta.Fill(Ds, "Define_Daramad")
            CmbOstan.Items.Clear()
            For i As Integer = 0 To Ds.Tables("Define_Daramad").Rows.Count - 1
                If CmbOstan.FindStringExact(Ds.Tables("Define_Daramad").Rows(i).Item("NamOne")) = -1 Then
                    CmbOstan.Items.Add(Ds.Tables("Define_Daramad").Rows(i).Item("NamOne"))
                End If
            Next
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Daramad_List", "GetDaramad")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub RefreshBank()
        Try
            ...
            Next
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Daramad_List", "RefreshBank")
        End Try
    End Sub
    ...
    Private Sub BtnNewP_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnNewP.Click
        Try
            Fnew = True
            Using FrmPeople As New DefineDaramad
                FrmPeople.ShowDialog()
            End Using
            Fnew = False
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Daramad_List", "BtnNewP_Click")
        End Try
    End Sub
