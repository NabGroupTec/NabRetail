...
Public Class RelationVisitorFrosh
    ...
    Private Sub RelationVisitorFrosh_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If EDIT <> 0 Then
            Try
                ...
                Using cmd As New SqlCommand("SELECT REPLACE(Listkala.Kala,'.','/') As Kala ,Idkala,Listkala.Id, Listkala.Mon, Define_Kala.Nam, Define_OneGroup.NamOne + '-' + Define_TwoGroup.NamTwo AS GroupKala FROM Listkala INNER JOIN Define_Kala ON Listkala.Idkala = Define_Kala.Id INNER JOIN Define_OneGroup ON Define_Kala.IdCodeOne = Define_OneGroup.Id INNER JOIN Define_TwoGroup ON Define_Kala.IdCodeTwo = Define_TwoGroup.Id AND Define_OneGroup.Id = Define_TwoGroup.IdOne WHERE IdLinkVisitor =" & EDIT, ConectionBank)
                ...
                For i As Integer = 0 To DGV1.RowCount - 2
                    dt.Clear()
                    Using cmd As New SqlCommand("SELECT REPLACE(ListDarsad.Frosh,'.','/') As Frosh, REPLACE(ListDarsad.Darsad,'.','/') AS Darsad, Define_Group_P.nam,ListDarsad.IdGroup FROM ListDarsad INNER JOIN Define_Group_P ON ListDarsad.IdGroup = Define_Group_P.Id WHERE IdlinkKala =" & DGV1.Item("Cln_Id", i).Value, ConectionBank)
                ...
                If ConectionBank.State <> ConnectionState.Open Then ConectionBank.Open()
                dtr = Nothing
                Using cmd As New SqlCommand("SELECT  REPLACE(ListOrderDarsad.Darsad,'.','/') As Darsad, Define_Group_P.nam,IdGroup FROM ListOrderDarsad INNER JOIN Define_Group_P ON ListOrderDarsad.IdGroup = Define_Group_P.Id WHERE IdLinkVisitor =" & EDIT, ConectionBank)
                    dtr = cmd.ExecuteReader
                End Using
                If dtr.HasRows Then
            ...
            Catch ex As Exception
                If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationVisitorFrosh", "RelationVisitorFrosh_Load")
                Me.Close()
            End Try
        End If
    End Sub
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        Try
            ...
            Me.Enabled = True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationVisitorFrosh", "BtnSave_Click")
        End Try
    End Sub
    ...
    Private Function AreYouAdd(ByVal Idvisitior As Long) As Boolean
        Try
            ...
            Using cmd As New SqlCommand("SELECT  COUNT(IdVisitor) FROM ListVisitor WHERE IdVisitor =" & Idvisitior, ConectionBank)
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationVisitorFrosh", "AreYouAdd")
            Return False
        End Try
    End Function
    ...
    Private Function AreYouEdit(ByVal NewId As Long, ByVal OldId As Long) As Boolean
        Try
            If ConectionBank.State = ConnectionState.Closed Then ConectionBank.Open()
            Using cmd As New SqlCommand("SELECT  COUNT(IdVisitor) FROM ListVisitor WHERE IdVisitor =" & NewId & " AND IdVisitor <> " & OldId, ConectionBank)
                If cmd.ExecuteScalar <= 0 Then
                    If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
                    Return True
                Else
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationVisitorFrosh", "AreYouEdit")
            Return False
        End Try
    End Function
    ...
    Private Function SaveList() As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("INSERT INTO ListVisitor(IdVisitor) VALUES(@IdVisitor)", ConectionBank)
                Cmd.Parameters.AddWithValue("@IdVisitor", SqlDbType.BigInt).Value = CLng(TxtIdVisitor.Text)
                Cmd.ExecuteNonQuery()
            End Using

            Using Cmd As New SqlCommand("INSERT INTO ListKala(IdKala,Mon,Kala,IdLinkVisitor) VALUES(@IdKala,@Mon,@Kala,@IdLinkVisitor);Select @@IDENTITY", ConectionBank)
                For i As Integer = 0 To DGV1.RowCount - 2
                    ...
                    Using CmdDarsad As New SqlCommand("INSERT INTO ListDarsad(Frosh,Darsad,IdLinkKala,IdGroup) VALUES(@Frosh,@Darsad,@IdLinkKala,@IdGroup)", ConectionBank)
                        For j As Integer = 0 To DGV2.RowCount - 2
                            If DGV2.Item("Cln_Row", j).Value = i Then
            ...
            Using CmdODarsad As New SqlCommand("INSERT INTO ListOrderDarsad(IdGroup,Darsad,IdLinkVisitor) VALUES(@IdGroup,@Darsad,@IdLinkVisitor)", ConectionBank)
                For j As Integer = 0 To DGV3.RowCount - 2
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), " رابطه ویزیتور و فروش هدف-کالا", "جدید", "تعیین فروش هدف ویزیتور :" & TxtVisitor.Text, "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationVisitorFrosh", "SaveList")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Function EditList() As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("DELETE FROM ListKala WHERE IdLinkVisitor=@IdVisitor;DELETE FROM  ListOrderDarsad WHERE IdLinkVisitor=@IdVisitor", ConectionBank)
                Cmd.Parameters.AddWithValue("@IdVisitor", SqlDbType.BigInt).Value = EDIT
                Cmd.ExecuteNonQuery()
            End Using

            Using Cmd As New SqlCommand("Update ListVisitor SET IdVisitor=@IdVisit WHERE IdVisitor=@IdVisitor", ConectionBank)
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), " رابطه ویزیتور و فروش هدف-کالا", "ویرایش", If(Tmp_String1 = TxtVisitor.Text, "ویرایش فروش هدف ویزیتور :" & TxtVisitor.Text, "ویرایش فروش هدف ویزیتور :" & Tmp_String1 & " به  " & TxtVisitor.Text), "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationVisitorFrosh", "EditList")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Sub BtnSelect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSelect.Click
        Using frm As New ListRelationVisit
            frm.ShowDialog()
            DGV1.AllowUserToAddRows = False
