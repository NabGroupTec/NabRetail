...
Public Class FrmReportFroshUser
    ...
    Private Sub GetUserList()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmReportFroshUser", "GetUserList")
        End Try
    End Sub
    ...
    Private Sub FrmSodNKhales_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Next
        Access_Form = Get_Access_Form("F101")
    ...
    Private Sub GetKala()
        Try
            ...
            DGV.DataSource = dv
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmReportFroshUser", "GetKala")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        ...
        End If
        If CmbBox.Text = "" Then
            MessageBox.Show("کاربری جهت تهیه گزارش انتخاب نشده است", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
            CmbBox.Focus()
            Exit Sub
        End If
        If ChkTime.Checked = True Then
        ...
        Else
            ...
            ElseIf ChkG.Checked = True Then
                ...
                FrmPrint.Lend = "SELECT ISNULL(SUM(Darsad),0) As Darsad FROM (SELECT ISNULL(SUM(DarsadMon),0) As Darsad FROM KalaFactorSell INNER JOIN ListFactorSell ON KalaFactorSell.IdFactor = ListFactorSell.IdFactor INNER JOIN Define_People ON ListFactorSell.IdName =Define_People.ID WHERE (ListFactorSell.Activ =1 AND ListFactorSell.IdUser =" & CmbBox.SelectedValue & " And ListFactorSell.Stat =3 AND DarsadMon >0) " & Listpeople & Group & dat & Part & " UNION ALL SELECT ISNULL(SUM(Discount),0) As Darsad FROM  ListFactorSell INNER JOIN Define_People ON ListFactorSell.IdName =Define_People.ID WHERE (ListFactorSell.Activ =1 AND ListFactorSell.IdUser =" & CmbBox.SelectedValue & " And ListFactorSell.Stat =3 AND Discount >0) " & Group & dat & Part & "  UNION ALL SELECT ISNULL(SUM(DarsadMon)* -1,0) As Darsad FROM KalaFactorBackSell  INNER JOIN ListFactorBackSell  ON KalaFactorBackSell .IdFactor = ListFactorBackSell .IdFactor INNER JOIN Define_People ON ListFactorBackSell.IdName =Define_People.ID WHERE (ListFactorBackSell .Activ =1 AND ListFactorBackSell .IdUser =" & CmbBox.SelectedValue & " And DarsadMon >0) " & Listpeople & Group & dat & Part2 & " UNION ALL SELECT ISNULL(SUM(Discount) * -1,0) As Darsad FROM  ListFactorBackSell INNER JOIN Define_People ON ListFactorBackSell.IdName =Define_People.ID  WHERE (ListFactorBackSell .Activ =1 AND ListFactorBackSell .IdUser =" & CmbBox.SelectedValue & "  AND Discount >0) " & Group & dat & Part2 & " ) AS AllDiscount"
                FrmPrint.Str2 = "SELECT ISNULL(SUM(KalaKasriFactor.Mon-KalaKasriFactor.DarsadMon),0) As Kasri FROM KalaKasriFactor INNER JOIN ListKasriFactor ON KalaKasriFactor.IdFactor = ListKasriFactor.IdFactor INNER JOIN ListFactorSell ON ListKasriFactor.IdFactor = ListFactorSell.IdFactor INNER JOIN Define_People ON ListFactorSell.IdName =Define_People .ID WHERE ListFactorSell.IdUser =" & CmbBox.SelectedValue & Replace(Listpeople, "IDKala", "IDR") & Group & dat2 & Part
                FrmPrint.CHoose = "FROSHUSER5"
            End If
        End If

        TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "فروش کاربر", "تهیه گزارش", "گزارش فروش کاربر:" & CmbBox.Text, "")
    ...
    Private Sub GetKey(ByVal e As KeyEventArgs)
        If e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "Rep_FroshKarbar.htm")
