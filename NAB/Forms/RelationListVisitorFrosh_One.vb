...
Public Class RelationListVisitorFrosh_One

    Dim ds As New DataSet
    Dim str As String = "SELECT  ListVisitor_OG.IdVisitor, Define_Visitor.Nam FROM ListVisitor_OG INNER JOIN Define_Visitor ON ListVisitor_OG.IdVisitor = Define_Visitor.Id"
    ...
    Private Sub Fill_KDgv(ByVal Idvisit As Long)
        Try
            DGV1.DataSource = Nothing
            dsK.Clear()
            strK = "SELECT REPLACE(Listkala_OG.Kala,'.','/') As Kala , Listkala_OG.Mon,Listkala_OG.IdKala,Listkala_OG.Id,Define_OneGroup .NamOne As nam FROM Listkala_OG INNER JOIN Define_OneGroup  ON Listkala_OG.Idkala = Define_OneGroup.Id WHERE IdLinkVisitor =" & Idvisit
            If Not dtaK Is Nothing Then
                dtaK.Dispose()
            End If
            '''''''''''''''''''''''''''
            dtaK = New SqlDataAdapter(strK, DataSource)
            dtaK.Fill(dsK, "Listkala_OG")
            DGV1.AutoGenerateColumns = False
            bsK.DataSource = dsK
            bsK.DataMember = "Listkala_OG"
            DGV1.DataSource = bsK
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_One", "fill_Kdgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Fill_DDgv(ByVal Idvisit As Long)
        Try
            DGV2.DataSource = Nothing
            dsD.Clear()
            strD = "SELECT REPLACE(ListDarsad_OG.Frosh,'.','/') As Frosh, REPLACE(ListDarsad_OG.Darsad,'.','/') AS Darsad, Define_Group_P.nam FROM ListDarsad_OG INNER JOIN Define_Group_P ON ListDarsad_OG.IdGroup = Define_Group_P.Id WHERE IdlinkKala =" & Idvisit
            If Not dtaD Is Nothing Then
                dtaD.Dispose()
            End If
            '''''''''''''''''''''''''''
            dtaD = New SqlDataAdapter(strD, DataSource)
            dtaD.Fill(dsD, "ListDarsad_OG")
            DGV2.AutoGenerateColumns = False
            bsD.DataSource = dsD
            bsD.DataMember = "ListDarsad_OG"
            DGV2.DataSource = bsD
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_One", "fill_Ddgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Fill_ODgv(ByVal Idvisit As Long)
        Try
            DGV4.DataSource = Nothing
            dsO.Clear()
            strO = "SELECT  REPLACE(ListOrderDarsad_OG.Darsad,'.','/') As Darsad, Define_Group_P.nam FROM ListOrderDarsad_OG INNER JOIN Define_Group_P ON ListOrderDarsad_OG.IdGroup = Define_Group_P.Id WHERE IdLinkVisitor =" & Idvisit
            If Not dtaO Is Nothing Then
                dtaO.Dispose()
            End If
            '''''''''''''''''''''''''''
            dtaO = New SqlDataAdapter(strO, DataSource)
            dtaO.Fill(dsO, "ListOrderDarsad_OG")
            DGV4.AutoGenerateColumns = False
            bsO.DataSource = dsO
            bsO.DataMember = "ListOrderDarsad_OG"
            DGV4.DataSource = bsO
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_One", "Fill_ODgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub SetButton()
        Try
            ...
            End If
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_One", "SetButton")
            Me.Close()
        End Try
    End Sub
    Private Sub Fill_Dgv()
        Try
            ...
            dta.Fill(ds, "ListVisitor_OG")
            DGV3.AutoGenerateColumns = False
            bs.DataSource = ds
            bs.DataMember = "ListVisitor_OG"
            DGV3.DataSource = bs
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_One", "fill_dgv")
            Me.Close()
        End Try
    End Sub
    Private Sub RefreshBank()
        Try
            ...
            dta.Fill(ds, "ListVisitor_OG")
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_One", "RefreshBank")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub cmdadd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdadd.Click
        Try
            Using FrmAdd As New RelationVisitorFrosh_One
                FrmAdd.EDIT = 0
                FrmAdd.ShowDialog()
            End Using
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_One", "cmdadd_Click")
            Me.RefreshBank()
        End Try
    End Sub
    ...
    Private Sub cmddel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmddel.Click
        Try
            ...
            If str = 6 Then
                ...
                Me.RefreshBank()
                TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "رابطه ویزیتور و فروش هدف-گروه اصلی", "حذف", "حذف فروش هدف ویزیتور :" & nam, "")
            End If
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_One", "cmddel_Click")
            Me.RefreshBank()
        End Try
    End Sub
    Private Function DeleteVisit(ByVal Id As Long) As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("DELETE FROM ListVisitor_OG WHERE IdVisitor=@IdVisitor", ConectionBank)
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_One", "DeleteVisit")
            End If
            Me.RefreshBank()
            Return False
        End Try
