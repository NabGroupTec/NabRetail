...
Public Class Anbar_List
    ...
    Private Sub GetAnbar()
        Try
            ...
            End If
            '''''''''''''''''''''''''''
            Dta = New SqlDataAdapter("If (SELECT COUNT(IdUser) FROM Define_UserRAnbar  WHERE IdUser =" & Id_User & ")>0 SELECT ID, nam FROM Define_Anbar WHERE SetDeActiv='False' AND Id in (SELECT IDA   FROM Define_UserRPAnbar   WHERE IdUser =" & Id_User & " ) else SELECT ID, nam FROM Define_Anbar WHERE SetDeActiv='False'", DataSource)
            Dta.Fill(Ds, "Define_Anbar")
            DGV.AutoGenerateColumns = False
            dv = Ds.Tables("Define_Anbar").DefaultView
            DGV.DataSource = dv
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Anbar_List", "GetAnbar")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub BtnNewP_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnNewP.Click
        Try
            ...
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Anbar_List", "BtnNewP_Click")
        End Try
    End Sub
