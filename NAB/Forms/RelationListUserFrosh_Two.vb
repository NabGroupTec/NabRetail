...
Public Class RelationListUserFrosh_Two

    Dim ds As New DataSet
    Dim str As String = "SELECT  ListUser_G.IdUser, Define_User.Nam FROM ListUser_G INNER JOIN Define_User ON ListUser_G.IdUser = Define_User.Id"
    ...
    Private Sub Fill_KDgv(ByVal Idvisit As Long)
        Try
            DGV1.DataSource = Nothing
            dsK.Clear()
            strK = "SELECT REPLACE(ListkalaU_G.Kala,'.','/') As Kala , ListkalaU_G.Mon,ListkalaU_G.IdKala,ListkalaU_G.Id,Define_TwoGroup .NamTwo As nam FROM ListkalaU_G INNER JOIN Define_TwoGroup  ON ListkalaU_G.Idkala = Define_TwoGroup.Id WHERE IdLinkUser =" & Idvisit
            If Not dtaK Is Nothing Then
                dtaK.Dispose()
            End If
            '''''''''''''''''''''''''''
            dtaK = New SqlDataAdapter(strK, DataSource)
            dtaK.Fill(dsK, "ListkalaU_G")
            DGV1.AutoGenerateColumns = False
            bsK.DataSource = dsK
            bsK.DataMember = "ListkalaU_G"
            DGV1.DataSource = bsK
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh_Two", "fill_Kdgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Fill_DDgv(ByVal Idvisit As Long)
        Try
            DGV2.DataSource = Nothing
            dsD.Clear()
            strD = "SELECT REPLACE(ListDarsadU_G.Frosh,'.','/') As Frosh, REPLACE(ListDarsadU_G.Darsad,'.','/') AS Darsad, Define_Group_P.nam FROM ListDarsadU_G INNER JOIN Define_Group_P ON ListDarsadU_G.IdGroup = Define_Group_P.Id WHERE IdlinkKala =" & Idvisit
            If Not dtaD Is Nothing Then
                dtaD.Dispose()
            End If
            '''''''''''''''''''''''''''
            dtaD = New SqlDataAdapter(strD, DataSource)
            dtaD.Fill(dsD, "ListDarsadU_G")
            DGV2.AutoGenerateColumns = False
            bsD.DataSource = dsD
            bsD.DataMember = "ListDarsadU_G"
            DGV2.DataSource = bsD
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh_Two", "fill_Ddgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Fill_ODgv(ByVal Idvisit As Long)
        Try
            DGV4.DataSource = Nothing
            dsO.Clear()
            strO = "SELECT  REPLACE(ListOrderDarsadU_G.Darsad,'.','/') As Darsad, Define_Group_P.nam FROM ListOrderDarsadU_G INNER JOIN Define_Group_P ON ListOrderDarsadU_G.IdGroup = Define_Group_P.Id WHERE IdLinkUser =" & Idvisit
            If Not dtaO Is Nothing Then
                dtaO.Dispose()
            End If
            '''''''''''''''''''''''''''
            dtaO = New SqlDataAdapter(strO, DataSource)
            dtaO.Fill(dsO, "ListOrderDarsadU_G")
            DGV4.AutoGenerateColumns = False
            bsO.DataSource = dsO
            bsO.DataMember = "ListOrderDarsadU_G"
            DGV4.DataSource = bsO
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh_Two", "Fill_ODgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub SetButton()
        Try
            ...
            End If
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh_Two", "SetButton")
            Me.Close()
        End Try
    End Sub
    Private Sub Fill_Dgv()
        Try
            ...
            dta.Fill(ds, "ListUser_G")
            DGV3.AutoGenerateColumns = False
            bs.DataSource = ds
            bs.DataMember = "ListUser_G"
            DGV3.DataSource = bs
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh_Two", "fill_dgv")
            Me.Close()
        End Try
    End Sub
    Private Sub RefreshBank()
        Try
            ...
            dta.Fill(ds, "ListUser_G")
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh_Two", "RefreshBank")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub cmdadd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdadd.Click
        Try
            Using FrmAdd As New RelationUserFrosh_Two
                FrmAdd.EDIT = 0
                FrmAdd.ShowDialog()
            End Using
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh_Two", "cmdadd_Click")
            Me.RefreshBank()
        End Try
    End Sub
    ...
    Private Sub cmddel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmddel.Click
        Try
            ...
            If str = 6 Then
                ...
                Me.RefreshBank()
                TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "رابطه کاربر و فروش هدف-گروه فرعی", "حذف", "حذف فروش هدف کاربر :" & nam, "")
            End If
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh_Two", "cmddel_Click")
            Me.RefreshBank()
        End Try
    End Sub
    Private Function DeleteUser(ByVal Id As Long) As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("DELETE FROM ListUser_G WHERE IdUser=@IdUser", ConectionBank)
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListUserFrosh_Two", "DeleteUser")
            End If
            Me.RefreshBank()
            Return False
        End Try
