...
Public Class Mobile_PayFactor
        ...
        Try
            ...
            If LState.Text <> "5" Then
                Using Cmd As New SqlCommand("INSERT Moein_People (D_date,disc,mon,T,IdPeople,IdUser,Type,Number_Type,Type_Sanad) VALUES(@D_date,@disc,@mon,@T,@IdPeople,@IdUser,@Type,@Number_Type,@Type_Sanad);SELECT @@IDENTITY", ConectionBank)
                    Cmd.Parameters.AddWithValue("@D_date", SqlDbType.NVarChar).Value = LDate.Text
                    Cmd.Parameters.AddWithValue("@Disc", SqlDbType.NVarChar).Value = "ف.فروش " & LIdFac.Text & If(String.IsNullOrEmpty(LHandNumber.Text.Trim), "", " شماره دستی " & LHandNumber.Text) & DiscFactor
            ...
            ''''''''''''''''
            Using Cmd As New SqlCommand("UPDATE  Mobile_ListFactorSell SET [Confirm]=2,Disc=@Disc,IdSell=@IdSell WHERE IdFactor =@IdFactor", ConectionBank)
                Cmd.Parameters.AddWithValue("@Disc", SqlDbType.NVarChar).Value = Tmp_String1
                Cmd.Parameters.AddWithValue("@IdSell", SqlDbType.BigInt).Value = CLng(LIdFac.Text)
                Cmd.Parameters.AddWithValue("@IdFactor", SqlDbType.BigInt).Value = CLng(LMFac.Text)
                Cmd.ExecuteNonQuery()
            End Using
            '//////////////////////////////////////////
    ...
    Private Sub ChKLend_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChKLend.CheckedChanged
        If ChKLend.Checked = True Then
            ...
            TxtTime.Text = Tmp2 'Rate 'GetTimePeople(LIdname.Text)
        Else
