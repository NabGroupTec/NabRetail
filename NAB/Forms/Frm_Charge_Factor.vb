...
Public Class Frm_Charge_Factor
    ...
    Private Sub Vosol_chk_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
    End Sub
    Private Sub ShowInfoCharge()
        Try
            Dim QueryKala As String = ""
            QueryKala = "SELECT ListFactorCharge.IdFactor, ListFactorBuy.D_date, Define_People.Nam FROM  ListFactorCharge INNER JOIN ListFactorBuy ON ListFactorCharge.IdFactor = ListFactorBuy.IdFactor INNER JOIN Define_People ON ListFactorBuy.IdName = Define_People.ID WHERE ListFactorCharge.Id=" & CLng(Lsanad.Text)
            ...
            Using SQLComanad As New SqlCommand(QueryKala, ConectionBank)
                ...
                Else
                    dtrInfo.Read()
                    TxtIDFac.Text = dtrInfo("IdFactor")
                    TxtPeople.Text = dtrInfo("Nam")
                    TxtDate.Text = dtrInfo("D_Date")
                End If
            End Using
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
        Catch ex As Exception
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Frm_Charge_Factor", "ShowInfoCharge")
            Me.Close()
        End Try
    End Sub
    Private Sub ShowCharge()
        Try
            Dim QueryKala As String = ""
            QueryKala = "SELECT  KalaFactorCharge.IdCharge, KalaFactorCharge.Mon,KalaFactorCharge.CDisc, Define_Charge.nam FROM  KalaFactorCharge INNER JOIN Define_Charge ON KalaFactorCharge.IdCharge = Define_Charge.ID WHERE IdSanad =" & CLng(Lsanad.Text)
            Dim dtrInfo As SqlDataReader = Nothing
            If ConectionBank.State <> ConnectionState.Open Then ConectionBank.Open()
            Using SQLComanad As New SqlCommand(QueryKala, ConectionBank)
                dtrInfo = SQLComanad.ExecuteReader
        ...
        Catch ex As Exception
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Frm_Charge_Factor", "ShowCharge")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Btnadd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btnadd.Click
        Try
            Btnadd.Focus()
            DGV2.EndEdit()
            If String.IsNullOrEmpty(TxtIDFac.Text) Or String.IsNullOrEmpty(TxtPeople.Text) Or String.IsNullOrEmpty(TxtDate.Text) Then
                MessageBox.Show("فاکتور خرید را انتخاب کنید", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
                TxtIDFac.Focus()
                Exit Sub
            End If
            ...
            For i As Integer = 0 To DGV2.RowCount - 2
                ...
                ChargeListArray(ChargeListArray.Length - 1).disc = TxtDisc.Text
                ChargeListArray(ChargeListArray.Length - 1).sanad = TxtSanad.Text
    ...
    Private Sub EditOperation()
        ...
        Using FrmPay As New Pay_FactorCharge
            FrmPay.LIdFac.Text = CLng(Lsanad.Text)
            FrmPay.TxtFacMon.Text = TxtAllMoney.Text
            FrmPay.LBuyFac.Text = CLng(TxtIDFac.Text)
            FrmPay.LFactor.Text = CLng(TxtIDFac.Text)
            FrmPay.LState.Text = 17
            FrmPay.LEdit.Text = "1"
            FrmPay.ShowDialog()
            If FrmPay.LOk.Text = "0" Then
                ...
                Exit Sub
            End If
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "هزینه فاکتور خرید", "ویرایش", "ویرایش سند:" & CLng(Lsanad.Text) & " شماره فاکتور: " & TxtIDFac.Text & " تعداد اقلام هزینه: " & DGV2.RowCount - 1 & " جمع کل هزینه: " & TxtAllMoney.Text, "")
            Me.Close()
        End Using
    ...
    Private Function SaveCharge() As Long
        ...
        Try
            ...
            Using Cmd As New SqlCommand("INSERT INTO  ListFactorCharge (D_date,IdFactor,IdUser,Disc,Activ,Discount,Cash,MonHavaleh,MonSellChk,MonPayChk,Lend,EditFlag,Sanad) VALUES(@D_date,@IdFactor,@IdUser,@Disc,@Activ,@Discount,@Cash,@MonHavaleh,@MonSellChk,@MonPayChk,@Lend,@EditFlag,@Sanad);SELECT @@IDENTITY", ConectionBank)
                Cmd.Parameters.AddWithValue("@D_date", SqlDbType.NVarChar).Value = GetDate()
                Cmd.Parameters.AddWithValue("@IdFactor", SqlDbType.BigInt).Value = CLng(TxtIDFac.Text)
            ...
            Using Cmd As New SqlCommand("INSERT INTO KalaFactorCharge (IdSanad,IdCharge,Mon,CDisc) VALUES(@IdSanad,@IdCharge,@Mon,@CDisc)", ConectionBank)
                For i As Integer = 0 To DGV2.RowCount - 2
        ...
        Catch ex As Exception
            ...
            If Err.Number = 5 Then
                MessageBox.Show("اطلاعات فاکتور هزینه خرید قابل ذخیره شدن نیست", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Frm_Charge_Factor", "SaveCharge")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return -1
        End Try
    ...
    Private Sub SaveOperation()
        Try
            ...
            Using FrmPay As New Pay_FactorCharge
                ...
                FrmPay.ShowDialog()
                If FrmPay.LOk.Text = "0" Then
                    RoolBackOthrCharge(IdFac, 17)
                    Me.Enabled = True
                    Exit Sub
                End If
                TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "هزینه فاکتور خرید", "جدید", "ثبت سند:" & IdFac & " شماره فاکتور: " & TxtIDFac.Text & " تعداد اقلام هزینه: " & DGV2.RowCount - 1 & " جمع کل هزینه: " & TxtAllMoney.Text, "")

            End Using
            Me.Enabled = True
            Me.Close()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Frm_Charge_Factor", "SaveOperation")
            Me.Enabled = True
        End Try
    End Sub
    ...
    Private Sub TxtIDFac_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TxtIDFac.KeyDown
        If e.KeyCode = Keys.Enter Then DGV2.Focus()
    End Sub
    ...
    Private Sub TxtIDFac_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TxtIDFac.KeyPress
        ...
        frmlk.str = "SELECT ListFactorBuy.IdFactor, ListFactorBuy.D_date, Define_People.Nam FROM  ListFactorBuy INNER JOIN Define_People ON ListFactorBuy .IdName =Define_People .ID WHERE ListFactorBuy.Stat =0 AND ListFactorBuy.Activ =1"
        frmlk.TxtSearch.Text = e.KeyChar()
        frmlk.ShowDialog()
        e.Handled = True
        TxtIDFac.Focus()
