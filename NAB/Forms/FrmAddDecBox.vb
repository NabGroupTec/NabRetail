...
Public Class FrmAddDecBox
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmAddDecBox", "BtnSave_Click")
        End Try
    End Sub
    Private Function SaveSanad() As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("INSERT INTO  Sanad_AddDecBox(D_Date,IdBox,Mon,Disc,IDsanadBox,IdUser,Flag) VALUES(@D_Date,@IdBox,@Mon,@Disc,@IDsanadBox,@IdUser,@Flag);SELECT @@IDENTITY", ConectionBank)
                ...
                Cmd.Parameters.AddWithValue("@IDsanadBox", SqlDbType.BigInt).Value = DBNull.Value
            ...
            Using Cmd As New SqlCommand("INSERT Moein_Box (D_date,disc,mon,T,IdBox,IdUser) VALUES(@D_date,@disc,@mon,@T,@IdBox,@IdUser);SELECT @@IDENTITY", ConectionBank)
                Cmd.Parameters.AddWithValue("@D_date", SqlDbType.NVarChar).Value = TxtPayDate.ThisText
                Cmd.Parameters.AddWithValue("@Disc", SqlDbType.NVarChar).Value = IIf(RdoAdd.Checked = True, "اضافات صندوق بابت سند" & Idsanad & "-" & TxtDiscT.Text.Trim, "کسورات صندوق بابت سند" & Idsanad & "-" & TxtDiscT.Text.Trim)
                Cmd.Parameters.AddWithValue("@mon", SqlDbType.BigInt).Value = CDbl(TxtMonT.Text)
            ...
            Using Cmd As New SqlCommand("UPDATE Sanad_AddDecBox SET IDsanadBox=@IDsanadBox WHERE Id=@Id", ConectionBank)
                Cmd.Parameters.AddWithValue("@IDsanadBox", SqlDbType.BigInt).Value = IdBox
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "اضافات و کسورات صندوق", "جدید", "ثبت سند :" & Idsanad & " نام صندوق:" & TxtBox.Text & " نوع سند:" & If(RdoAdd.Checked = True, "اضافات", "کسورات") & " مبلغ:" & TxtMonT.Text, "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmAddDecBox", "SaveSanad")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Function EditSanad() As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("UPDATE   Sanad_AddDecBox SET D_Date=@D_Date,IdBox=@IdBox,Mon=@Mon,Disc=@Disc,IdUser=@IdUser,Flag=@Flag WHERE Id=@Id", ConectionBank)
                Cmd.Parameters.AddWithValue("@D_date", SqlDbType.NVarChar).Value = TxtPayDate.ThisText
                Cmd.Parameters.AddWithValue("@IDBox", SqlDbType.BigInt).Value = CLng(TxtIdBox.Text)
                Cmd.Parameters.AddWithValue("@Mon", SqlDbType.BigInt).Value = CDbl(TxtMonT.Text)
                Cmd.Parameters.AddWithValue("@Disc", SqlDbType.NVarChar).Value = TxtDiscT.Text
            ...
            Using Cmd As New SqlCommand("UPDATE Moein_Box SET D_date=@D_date,disc=@disc,mon=@mon,T=@T,IdBox=@IdBox,IdUser=@IdUser WHERE Id=@ID", ConectionBank)
                Cmd.Parameters.AddWithValue("@D_date", SqlDbType.NVarChar).Value = TxtPayDate.ThisText
                Cmd.Parameters.AddWithValue("@Disc", SqlDbType.NVarChar).Value = IIf(RdoAdd.Checked = True, "اضافات صندوق بابت سند" & CLng(LSanad.Text) & "-" & TxtDiscT.Text.Trim, "کسورات صندوق بابت سند" & CLng(LSanad.Text) & "-" & TxtDiscT.Text.Trim)
                Cmd.Parameters.AddWithValue("@mon", SqlDbType.BigInt).Value = CDbl(TxtMonT.Text)
                Cmd.Parameters.AddWithValue("@T", SqlDbType.BigInt).Value = If(RdoAdd.Checked = True, 0, 1)
                Cmd.Parameters.AddWithValue("@IDBox", SqlDbType.BigInt).Value = CLng(TxtIdBox.Text)
                Cmd.Parameters.AddWithValue("@IdUser", SqlDbType.BigInt).Value = IIf(Id_User = 0, DBNull.Value, Id_User)
                Cmd.Parameters.AddWithValue("@ID", SqlDbType.BigInt).Value = dt.Rows(0).Item("IDsanadBox")
                Cmd.ExecuteNonQuery()
            End Using
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "اضافات و کسورات صندوق", "ویرایش", "ویرایش سند :" & CLng(LSanad.Text) & " نام صندوق:" & TxtBox.Text & " نوع سند:" & If(RdoAdd.Checked = True, "اضافات", "کسورات") & " مبلغ:" & TxtMonT.Text, "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmAddDecBox", "EditSanad")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Sub GetInfoSanad(ByVal id As Long)
        Try
            If ConectionBank.State = ConnectionState.Closed Then ConectionBank.Open()
            '////////////////////////////////////////
            Using Cmd As New SqlCommand("SELECT  Sanad_AddDecBox.Id,Sanad_AddDecBox.IdBox ,Sanad_AddDecBox.IDsanadBox , Sanad_AddDecBox.Mon, Sanad_AddDecBox.D_Date, Sanad_AddDecBox.Disc, Define_Box.nam, Sanad_AddDecBox.Flag FROM  Sanad_AddDecBox INNER JOIN Define_Box ON Sanad_AddDecBox.IdBox = Define_Box.ID WHERE Sanad_AddDecBox.Id=@ID", ConectionBank)
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    TxtIdBox.Text = dt.Rows(0).Item("IdBox")
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmAddDecBox", "GetInfoSanad")
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub GetKey(ByVal e As KeyEventArgs)
        ...
        ElseIf e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "AddDecBox.htm")
