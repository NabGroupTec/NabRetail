...
Public Class bank_List
    ...
    Private Sub GetBank()
        Try
            ...
            End If
            '''''''''''''''''''''''''''
            Dta = New SqlDataAdapter("SELECT ID,N_bank,Shobeh,Number_N,State,nam FROM Define_Bank " & LState.Text.Trim, DataSource)
    ...
    Private Sub RefreshBank()
        Try
            ...
            TxtSearch.Text = ""
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Anbar_List", "RefreshBank")
        End Try
    End Sub
        ...
        Try
            ...
            Else
                If RdoAval.Checked = True Then
                    dv.RowFilter = "N_Bank Like '" & TxtSearch.Text.Trim & "%'"
            ...
            ElseIf e.KeyCode = Keys.Enter Then
                Try
                    If DGV.RowCount > 0 Then
                        ...
                        Tmp_OneGroup = DGV.Item("Cln_number", DGV.CurrentRow.Index).Value.ToString
                        Tmp_TwoGroup = DGV.Item("Cln_Shobeh", DGV.CurrentRow.Index).Value.ToString
                        TMP_String1 = DGV.Item("Cln_People", DGV.CurrentRow.Index).Value.ToString
                        Me.Close()
                    End If
                Catch ex As Exception
                    IdKala = 0
    ...
    Private Sub BtnNewP_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnNewP.Click
        Try
            Fnew = True
            Using FrmAnbar As New DefineBank
                FrmAnbar.ShowDialog()
            End Using
            Fnew = False
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Bank_List", "BtnNewP_Click")
        End Try
    End Sub
