...
Public Class RelationUserFrosh_One
    ...
    Private Sub RelationVisitorFrosh_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If EDIT <> 0 Then
            Try
                ...
                Using cmd As New SqlCommand("SELECT REPLACE(ListKalaU_OG.Kala,'.','/') As Kala ,Idkala,ListKalaU_OG.Id, ListKalaU_OG.Mon, Define_OneGroup.NamOne  As Nam FROM ListKalaU_OG  INNER JOIN Define_OneGroup ON ListKalaU_OG.IdKala = Define_OneGroup.Id WHERE IdLinkUser =" & EDIT, ConectionBank)
                ...
                For i As Integer = 0 To DGV1.RowCount - 2
                    dt.Clear()
                    Using cmd As New SqlCommand("SELECT REPLACE(ListDarsadU_OG.Frosh,'.','/') As Frosh, REPLACE(ListDarsadU_OG.Darsad,'.','/') AS Darsad, Define_Group_P.nam,ListDarsadU_OG.IdGroup FROM ListDarsadU_OG INNER JOIN Define_Group_P ON ListDarsadU_OG.IdGroup = Define_Group_P.Id WHERE IdlinkKala =" & DGV1.Item("Cln_Id", i).Value, ConectionBank)
                ...
                If ConectionBank.State <> ConnectionState.Open Then ConectionBank.Open()
                dtr = Nothing
                Using cmd As New SqlCommand("SELECT  REPLACE(ListOrderDarsadU_OG.Darsad,'.','/') As Darsad, Define_Group_P.nam,IdGroup FROM ListOrderDarsadU_OG INNER JOIN Define_Group_P ON ListOrderDarsadU_OG.IdGroup = Define_Group_P.Id WHERE IdLinkUser =" & EDIT, ConectionBank)
                    dtr = cmd.ExecuteReader
                End Using
                If dtr.HasRows Then
            ...
            Catch ex As Exception
                If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh_One", "RelationVisitorFrosh_Load")
                Me.Close()
            End Try
        End If
    End Sub
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        Try
            ...
            Me.Enabled = True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh_One", "BtnSave_Click")
        End Try
    End Sub
    ...
    Private Function AreYouAdd(ByVal Idvisitior As Long) As Boolean
        Try
            ...
            Using cmd As New SqlCommand("SELECT  COUNT(IdUser) FROM ListUser_OG WHERE IdUser =" & Idvisitior, ConectionBank)
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh_One", "AreYouAdd")
            Return False
        End Try
    End Function
    ...
    Private Function AreYouEdit(ByVal NewId As Long, ByVal OldId As Long) As Boolean
        Try
            If ConectionBank.State = ConnectionState.Closed Then ConectionBank.Open()
            Using cmd As New SqlCommand("SELECT  COUNT(IdUser) FROM ListUser_OG WHERE IdUser =" & NewId & " AND IdUser <> " & OldId, ConectionBank)
                If cmd.ExecuteScalar <= 0 Then
                    If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
                    Return True
                Else
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh_One", "AreYouEdit")
            Return False
        End Try
    End Function
    ...
    Private Function SaveList() As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("INSERT INTO ListUser_OG(IdUser) VALUES(@IdUser)", ConectionBank)
                Cmd.Parameters.AddWithValue("@IdUser", SqlDbType.BigInt).Value = CLng(TxtIdVisitor.Text)
                Cmd.ExecuteNonQuery()
            End Using

            Using Cmd As New SqlCommand("INSERT INTO ListKalaU_OG(IdKala,Mon,Kala,IdLinkUser) VALUES(@IdKala,@Mon,@Kala,@IdLinkUser);Select @@IDENTITY", ConectionBank)
                For i As Integer = 0 To DGV1.RowCount - 2
                    ...
                    Using CmdDarsad As New SqlCommand("INSERT INTO ListDarsadU_OG(Frosh,Darsad,IdLinkKala,IdGroup) VALUES(@Frosh,@Darsad,@IdLinkKala,@IdGroup)", ConectionBank)
                        For j As Integer = 0 To DGV2.RowCount - 2
                            If DGV2.Item("Cln_Row", j).Value = i Then
            ...
            Using CmdODarsad As New SqlCommand("INSERT INTO ListOrderDarsadU_OG(IdGroup,Darsad,IdLinkUser) VALUES(@IdGroup,@Darsad,@IdLinkUser)", ConectionBank)
                For j As Integer = 0 To DGV3.RowCount - 2
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "رابطه کاربر و فروش هدف-گروه اصلی", "جدید", "تعیین فروش هدف کاربر :" & TxtVisitor.Text, "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh_One", "SaveList")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Function EditList() As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("DELETE FROM ListKalaU_OG WHERE IdLinkUser=@IdUser;DELETE FROM  ListOrderDarsadU_OG WHERE IdLinkUser=@IdUser", ConectionBank)
                Cmd.Parameters.AddWithValue("@IdUser", SqlDbType.BigInt).Value = EDIT
                Cmd.ExecuteNonQuery()
            End Using

            Using Cmd As New SqlCommand("Update ListUser_OG SET IdUser=@IdVisit WHERE IdUser=@IdUser", ConectionBank)
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), " رابطه کاربر و فروش هدف-گروه اصلی", "ویرایش", If(Tmp_String1 = TxtVisitor.Text, "ویرایش فروش هدف کاربر :" & TxtVisitor.Text, "ویرایش فروش هدف کاربر :" & Tmp_String1 & " به  " & TxtVisitor.Text), "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh_One", "EditList")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Sub BtnSelect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSelect.Click
        Using frm As New ListRelationUser_One
            frm.ShowDialog()
            DGV1.AllowUserToAddRows = False
