...
Public Class FrmFactor
    ...
    Private Sub TxtName_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TxtName.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            If ChkVisitor.Enabled = False Then
                TxtSanad.Focus()
            Else
                ChkVisitor.Focus()
            End If
    ...
    Private Function OkEnd() As Boolean
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "OkEnd")
            Return False
        End Try
    End Function
    ...
    Private Function SetKalaFacArray() As Boolean
        Try
            ...
            Return True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "SetKalaFacArray")
            Return False
        End Try
    End Function
    Private Function SetInfoFacArray() As Boolean
        Try
            ...
            Return True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "SetInfoFacArray")
            Return False
        End Try
    End Function
    Private Function RasKalaFac(ByVal Kind As String, ByVal IdP As Long, ByVal IdG As Long) As Long
        ...
        Catch ex As Exception
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "RasKalaFac")
            Return 0
        End Try
    End Function
    ...
    Private Function EditFactor(ByVal state As Long) As Boolean
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "EditFactor")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Function SaveFactor(ByVal state As Long) As Long
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "SaveFactor")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return -1
        End Try
            ...
            If DGV1.CurrentCell.ColumnIndex = 1 Then
                ...
                Else
                    ...
                    If LEdit.Text <> "0" Then
                        ...
                        If (CmbFac.Text = CmbFac.Items(0) Or CmbFac.Text = CmbFac.Items(4)) And Not Trim(DGV1.Item("Cln_name", DGV1.CurrentRow.Index).Value).Equals(String.Empty) Then
                            If Not (DGV1.Item("Cln_OldKol", DGV1.CurrentRow.Index).Value Is DBNull.Value Or DGV1.Item("Cln_OldJoz", DGV1.CurrentRow.Index).Value Is DBNull.Value Or DGV1.Item("Cln_OldAnbar", DGV1.CurrentRow.Index).Value Is DBNull.Value) Then
                    ...
                    If Tmp_Namkala <> "" Then
                        ...
                        ElseIf CmbFac.Text = CmbFac.Items.Item(0) Then
                            DGV1.Item("Cln_Fe", DGV1.CurrentRow.Index).Value = GetCostKharid(IdKala)
                            IIf(String.IsNullOrEmpty(DGV1.Item("Cln_Fe", DGV1.CurrentRow.Index).Value), 0, DGV1.Item("Cln_Fe", DGV1.CurrentRow.Index).Value)
                        Else
                            DGV1.Item("Cln_Fe", DGV1.CurrentRow.Index).Value = 0
                        End If
    ...
    Private Sub BtnServiceKala_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnServiceKala.Click
        Try
            ...
            End If
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "BtnServiceKala_Click")
        End Try
    End Sub
    Private Function ValidBuy() As Boolean
        Try
            ...
            Return True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "ValidBuy")
            Return False
        End Try
    End Function
    Private Function AreYouBackFac(ByVal IdFac As Long) As Boolean
        ...
        Catch ex As Exception
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "AreYouBackFac")
            Return False
        End Try
    End Function
    Private Function ValidSell() As Boolean
        Try
            ...
            Return True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "ValidSell")
            Return False
        End Try
    End Function
    Private Function ValidService() As Boolean
        Try
            ...
            Return True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "ValidService")
            Return False
        End Try
    End Function
    Private Function ValidDamage() As Boolean
        Try
            ...
            Return True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "ValidDamage")
            Return False
        End Try
    End Function
    ...
    Private Sub FactorFish()
        Try
            PrintSQl = "SELECT  PeopleMon=(SELECT ISNULL(EndCost,0) FROM [Define_CostKala] INNER JOIN Define_CityCostkala ON Define_CityCostkala.IdCity =[Define_CostKala].IdCity WHERE IdKala =KalaFactorSell.IdKala  AND Define_CityCostkala.IdCity =(SELECT IdCity FROM Define_People WHERE Id=ListFactorSell.IdName)),KalaFactorSell.KolCount, KalaFactorSell.JozCount, KalaFactorSell.Fe, KalaFactorSell.DarsadDiscount, KalaFactorSell.DarsadMon, KalaFactorSell.Mon,KalaFactorSell.KalaDisc, Define_Kala.Nam as NamKala, Define_Anbar.nam AS NamAnbar, Define_OneGroup.NamOne + '-' + Define_TwoGroup.NamTwo as GroupKala,Define_Vahed .Nam As Vahed FROM  ListFactorSell INNER JOIN KalaFactorSell ON ListFactorSell.IdFactor = KalaFactorSell.IdFactor INNER JOIN Define_Kala ON KalaFactorSell.IdKala = Define_Kala.Id INNER JOIN Define_Anbar ON KalaFactorSell.IdAnbar = Define_Anbar.ID INNER JOIN Define_OneGroup ON Define_Kala.IdCodeOne = Define_OneGroup.Id INNER JOIN Define_TwoGroup ON Define_Kala.IdCodeTwo = Define_TwoGroup.Id AND Define_OneGroup.Id = Define_TwoGroup.IdOne INNER JOIN Define_Vahed  ON Define_Kala.IdVahed  = Define_Vahed .Id WHERE ListFactorSell.IdFactor =" & CLng(Idfactor) & " UNION ALL SELECT PeopleMon=0,KalaFactorSell.KolCount, KalaFactorSell.JozCount, KalaFactorSell.Fe, KalaFactorSell.DarsadDiscount, KalaFactorSell.DarsadMon, KalaFactorSell.Mon,KalaFactorSell.KalaDisc, Define_Service.Nam as NamKala, NamAnbar='', GroupKala=N'کالای خدماتی',Vahed=N'خدمات' FROM ListFactorSell INNER JOIN KalaFactorSell ON ListFactorSell.IdFactor = KalaFactorSell.IdFactor INNER JOIN Define_Service  ON KalaFactorSell.IdService  = Define_Service .ID  WHERE ListFactorSell.IdFactor =" & CLng(Idfactor) & " Order By NamKala "
            Dim Dataret As New DataSetFactor
            Dataret.Clear()
            Using dbDA As New System.Data.SqlClient.SqlDataAdapter(PrintSQl, DataSource)
                dbDA.Fill(Dataret.DataTable1)
            ...
            If state = "3" Or state = "5" Then
                QueryInfo = "If (SELECT IdVisitor FROM ListFactorSell WHERE  ListFactorSell.IdFactor =" & CLng(Idfactor) & ")  Is NULL BEGIN SELECT  ListFactorSell.IdName AS CodeP,ListFactorSell.MonAdd ,ListFactorSell.MonDec ,ListFactorSell.Discount ,ListFactorSell.Cash ,ListFactorSell.MonHavaleh ,ListFactorSell.MonPayChk , ListFactorSell.D_date,ListFactorSell.IdUser , ISNULL(ListFactorSell.Disc,'') AS Disc, ISNULL(Define_People.NamFac,'')+ ' ' + Define_People.Nam As Nam,Define_People.[Address],(ISNULL(Define_People.Tell1,'') + ' - ' + ISNULL(Define_People.Tell2,'')) As Tell,Define_Ostan.NamO ,Define_City .NamCI ,Define_Part .NamP  ,ListFactorSell.IdVisitor,NamVisit='' FROM ListFactorSell INNER JOIN Define_People ON ListFactorSell.IdName = Define_People.ID INNER JOIN Define_Ostan ON Define_Ostan.Code =Define_People .IdOstan INNER JOIN Define_City ON Define_City .Code =Define_People .IdCity INNER JOIN Define_Part ON Define_Part .Code =Define_People .IdPart  WHERE   ListFactorSell.IdFactor =" & CLng(Idfactor) & "  END ELSE BEGIN SELECT  ListFactorSell.IdName AS CodeP,ListFactorSell.MonAdd ,ListFactorSell.MonDec ,ListFactorSell.Discount ,ListFactorSell.Cash ,ListFactorSell.MonHavaleh ,ListFactorSell.MonPayChk ,ListFactorSell.D_date,ListFactorSell.IdUser, ListFactorSell.Disc, ISNULL(Define_People.NamFac,'')+ ' ' + Define_People.Nam As Nam,Define_People.[Address],(ISNULL(Define_People.Tell1,'') + ' - ' + ISNULL(Define_People.Tell2,'')) As Tell ,Define_Ostan.NamO ,Define_City .NamCI ,Define_Part .NamP ,ListFactorSell.IdVisitor,Define_Visitor.Nam as NamVisit FROM   ListFactorSell INNER JOIN Define_People ON ListFactorSell.IdName = Define_People.ID INNER JOIN Define_Visitor  ON ListFactorSell .IdVisitor =Define_Visitor .Id  INNER JOIN Define_Ostan ON Define_Ostan.Code =Define_People .IdOstan INNER JOIN Define_City ON Define_City .Code =Define_People .IdCity INNER JOIN Define_Part ON Define_Part .Code =Define_People .IdPart WHERE  ListFactorSell.IdFactor =" & CLng(Idfactor) & "  END"
            End If
    ...
    Private Sub RefreshMoney(ByVal i As Integer)
        Try
            ...
            Me.CalDarsad()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "RefreshMoney")
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
    ...
    Private Sub BtnDiscount_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDiscount.Click
        Try
            ...
            BtnDiscount.Enabled = False
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "BtnDiscount_Click")
        End Try
    End Sub
    ...
    Private Function AreQustionForDiscount() As Boolean
        Try
            ...
            Return True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "AreQustionForDiscount")
            Return False
        End Try
    End Function
    ...
    Private Sub BtnSback_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnSback.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "BtnSback_Click")
        End Try
    End Sub
    ...
    Private Sub BtnKalaDiscount_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnKalaDiscount.Click
        Try
            ...
            BtnKalaDiscount.Enabled = False
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "BtnKalaDiscount_Click")
        End Try
    End Sub
    ...
    Private Function AreQustionForDiscountKala() As Boolean
        ...
        Catch ex As Exception
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "AreQustionForDiscountKala")
            Return False
        End Try
    End Function
    Private Function GetCoding(ByVal Id As Long) As String
        ...
        Catch ex As Exception
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "GetCoding")
            Return ""
        End Try
    End Function
    Private Sub CalKalaDiscount(ByVal ListDiscount() As KalaDiscount)
        ...
        Catch ex As Exception
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "CalKalaDiscount")
        End Try
    End Sub
    ...
    Private Function AreCalKalaDiscount(ByVal ListDiscount() As KalaDiscount) As Boolean
        ...
        Catch ex As Exception
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "AreCalKalaDiscount")
            Return False
        End Try
    End Function
    ...
    Private Sub BtnSCost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSCost.Click
        Try
            ...
            BtnSCost.Enabled = False
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "BtnSCost_Click")
        End Try
    End Sub
    ...
    Private Function AreQustionForSCost() As Boolean
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "AreQustionForSCost")
            Return False
        End Try
    End Function
    Private Sub CalScost(ByVal ListDiscount() As KalaDiscount, ByVal IdGroup As Long)
        ...
        Catch ex As Exception
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "CalSCost")
        End Try
    End Sub
    Private Function AreCalScost(ByVal ListDiscount() As KalaDiscount, ByVal IdGroup As Long) As Boolean
        ...
        Catch ex As Exception
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmFactor", "AreCalScost")
            Return False
        End Try
    End Function
End Class