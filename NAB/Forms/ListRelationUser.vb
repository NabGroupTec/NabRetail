...
Public Class ListRelationUser
    ...
    Private Sub Fill_KDgv(ByVal Idvisit As Long)
        ...
        Catch ex As Exception
            ...
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "ListRelationUser", "fill_Kdgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Fill_DDgv(ByVal Idvisit As Long)
        Try
            DGV2.DataSource = Nothing
            dsD.Clear()
            strD = "SELECT REPLACE(ListDarsadU.Frosh,'.','/') As Frosh, REPLACE(ListDarsadU.Darsad,'.','/') AS Darsad, Define_Group_P.nam,ListDarsadU.IdGroup FROM ListDarsadU INNER JOIN Define_Group_P ON ListDarsadU.IdGroup = Define_Group_P.Id WHERE IdlinkKala =" & Idvisit
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "ListRelationUser", "fill_Ddgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Fill_ODgv(ByVal Idvisit As Long)
        Try
            DGV4.DataSource = Nothing
            dsO.Clear()
            strO = "SELECT  REPLACE(ListOrderDarsadU.Darsad,'.','/') As Darsad, Define_Group_P.nam,IdGroup FROM ListOrderDarsadU INNER JOIN Define_Group_P ON ListOrderDarsadU.IdGroup = Define_Group_P.Id WHERE IdLinkUser =" & Idvisit
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "ListRelationUser", "Fill_ODgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Fill_Dgv()
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "ListRelationUser", "fill_dgv")
            Me.Close()
        End Try
    End Sub
    Private Sub RefreshBank()
        Try
            ...
            dta.Fill(ds, "ListUser")
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "ListRelationUser", "RefreshBank")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub BtnSelect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSelect.Click
        ...
        For i As Integer = 0 To DGV1.RowCount - 1
            dt.Clear()
            Using CMD As New SqlCommand("SELECT REPLACE(ListDarsadU.Frosh,'.','/') As Frosh, REPLACE(ListDarsadU.Darsad,'.','/') AS Darsad, Define_Group_P.nam,ListDarsadU.IdGroup FROM ListDarsadU INNER JOIN Define_Group_P ON ListDarsadU.IdGroup = Define_Group_P.Id WHERE IdlinkKala =" & CLng(DGV1.Item("Cln_Code", i).Value), ConectionBank)
