...
Public Class RelationVisitorFrosh_Two
    ...
    Private Sub RelationVisitorFrosh_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If EDIT <> 0 Then
            Try
                ...
                Using cmd As New SqlCommand("SELECT REPLACE(Listkala_G.Kala,'.','/') As Kala ,Idkala,Listkala_G.Id, Listkala_G.Mon, Define_OneGroup.NamOne AS GroupKala ,Define_TwoGroup.NamTwo As Nam FROM Listkala_G  INNER JOIN Define_TwoGroup ON Listkala_G.Idkala  = Define_TwoGroup.Id INNER JOIN Define_OneGroup ON Define_TwoGroup .IdOne = Define_OneGroup.Id WHERE IdLinkVisitor =" & EDIT, ConectionBank)
                ...
                For i As Integer = 0 To DGV1.RowCount - 2
                    dt.Clear()
                    Using cmd As New SqlCommand("SELECT REPLACE(ListDarsad_G.Frosh,'.','/') As Frosh, REPLACE(ListDarsad_G.Darsad,'.','/') AS Darsad, Define_Group_P.nam,ListDarsad_G.IdGroup FROM ListDarsad_G INNER JOIN Define_Group_P ON ListDarsad_G.IdGroup = Define_Group_P.Id WHERE IdlinkKala =" & DGV1.Item("Cln_Id", i).Value, ConectionBank)
                ...
                If ConectionBank.State <> ConnectionState.Open Then ConectionBank.Open()
                dtr = Nothing
                Using cmd As New SqlCommand("SELECT  REPLACE(ListOrderDarsad_G.Darsad,'.','/') As Darsad, Define_Group_P.nam,IdGroup FROM ListOrderDarsad_G INNER JOIN Define_Group_P ON ListOrderDarsad_G.IdGroup = Define_Group_P.Id WHERE IdLinkVisitor =" & EDIT, ConectionBank)
                    dtr = cmd.ExecuteReader
                End Using
                If dtr.HasRows Then
            ...
            Catch ex As Exception
                If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationVisitorFrosh_Two", "RelationVisitorFrosh_Load")
                Me.Close()
            End Try
        End If
    End Sub
    ...
    Private Sub GetKey(ByVal e As KeyEventArgs)
        If e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "ByFariKala.htm")
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        Try
            ...
            Me.Enabled = True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationVisitorFrosh_Two", "BtnSave_Click")
        End Try
    End Sub
    ...
    Private Function AreYouAdd(ByVal Idvisitior As Long) As Boolean
        Try
            ...
            Using cmd As New SqlCommand("SELECT  COUNT(IdVisitor) FROM ListVisitor_G WHERE IdVisitor =" & Idvisitior, ConectionBank)
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationVisitorFrosh_Two", "AreYouAdd")
            Return False
        End Try
    End Function
    ...
    Private Function AreYouEdit(ByVal NewId As Long, ByVal OldId As Long) As Boolean
        Try
            If ConectionBank.State = ConnectionState.Closed Then ConectionBank.Open()
            Using cmd As New SqlCommand("SELECT  COUNT(IdVisitor) FROM ListVisitor_G WHERE IdVisitor =" & NewId & " AND IdVisitor <> " & OldId, ConectionBank)
                If cmd.ExecuteScalar <= 0 Then
                    If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
                    Return True
                Else
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationVisitorFrosh_Two", "AreYouEdit")
            Return False
        End Try
    End Function
    ...
    Private Function SaveList() As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("INSERT INTO ListVisitor_G(IdVisitor) VALUES(@IdVisitor)", ConectionBank)
                Cmd.Parameters.AddWithValue("@IdVisitor", SqlDbType.BigInt).Value = CLng(TxtIdVisitor.Text)
                Cmd.ExecuteNonQuery()
            End Using

            Using Cmd As New SqlCommand("INSERT INTO ListKala_G(IdKala,Mon,Kala,IdLinkVisitor) VALUES(@IdKala,@Mon,@Kala,@IdLinkVisitor);Select @@IDENTITY", ConectionBank)
                For i As Integer = 0 To DGV1.RowCount - 2
                    ...
                    Using CmdDarsad As New SqlCommand("INSERT INTO ListDarsad_G(Frosh,Darsad,IdLinkKala,IdGroup) VALUES(@Frosh,@Darsad,@IdLinkKala,@IdGroup)", ConectionBank)
                        For j As Integer = 0 To DGV2.RowCount - 2
                            If DGV2.Item("Cln_Row", j).Value = i Then
            ...
            Using CmdODarsad As New SqlCommand("INSERT INTO ListOrderDarsad_G(IdGroup,Darsad,IdLinkVisitor) VALUES(@IdGroup,@Darsad,@IdLinkVisitor)", ConectionBank)
                For j As Integer = 0 To DGV3.RowCount - 2
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "رابطه ویزیتور و فروش هدف-گروه فرعی", "جدید", "تعیین فروش هدف ویزیتور :" & TxtVisitor.Text, "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationVisitorFrosh_Two", "SaveList")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Function EditList() As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("DELETE FROM ListKala_G WHERE IdLinkVisitor=@IdVisitor;DELETE FROM  ListOrderDarsad_G WHERE IdLinkVisitor=@IdVisitor", ConectionBank)
                Cmd.Parameters.AddWithValue("@IdVisitor", SqlDbType.BigInt).Value = EDIT
                Cmd.ExecuteNonQuery()
            End Using

            Using Cmd As New SqlCommand("Update ListVisitor_G SET IdVisitor=@IdVisit WHERE IdVisitor=@IdVisitor", ConectionBank)
            ...
            sqltransaction.Dispose()
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), " رابطه ویزیتور و فروش هدف-گروه فرعی", "ویرایش", If(Tmp_String1 = TxtVisitor.Text, "ویرایش فروش هدف ویزیتور :" & TxtVisitor.Text, "ویرایش فروش هدف ویزیتور :" & Tmp_String1 & " به  " & TxtVisitor.Text), "")
            Return True
        Catch ex As Exception
            sqltransaction.Rollback()
            sqltransaction.Dispose()
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationVisitorFrosh_Two", "EditList")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Sub BtnSelect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSelect.Click
        Using frm As New ListRelationVisit_Two
            frm.ShowDialog()
            DGV1.AllowUserToAddRows = False
