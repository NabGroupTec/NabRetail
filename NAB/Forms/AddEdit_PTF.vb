...
Public Class AddEdit_PTF
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "AddEdit_PTF", "BtnSave_Click")
            Me.RefreshBank()
        End Try
    End Sub
    Private Function CheckPeopleRepeate(ByVal IdUser As Long) As Boolean
        Try
            ...
            Dim Str As String = "SELECT Id,IdUser,IDP FROM Define_UserRP"
            Ds.Clear()
            ...
            Dta.Fill(Ds, "Define_UserRP")
            For i As Integer = 0 To DGV.RowCount - 2
                Dim dr() As DataRow = Ds.Tables("Define_UserRP").Select("IdUser=" & IdUser & " AND IDP=" & DGV.Item("Cln_IdP", i).Value)
                If dr.Length > 0 Then
                    MessageBox.Show(" طرف حساب سطر شماره  " & "{" & i + 1 & "}" & " قبلا برای این کاربر ثبت شده است", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Return False
                End If
            Next
            Return True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "AddEdit_PTF", "CheckPeopleRepeate")
            Return False
        End Try
    ...
    Private Function SavePeople(ByVal IdUser As Long) As Boolean
        ...
        Try
            ...
            Using CmdSelect As New SqlCommand("SELECT Count(IdUser) FROM Define_UserR WHERE IdUser=@IdUser", ConectionBank)
                CmdSelect.Parameters.AddWithValue("@IdUser", SqlDbType.BigInt).Value = IdUser
                If CmdSelect.ExecuteScalar() <= 0 Then
                    Using Cmd As New SqlCommand("INSERT INTO Define_UserR(IdUser) VALUES(@IdUser)", ConectionBank)
            ...
            Using Cmd As New SqlCommand("INSERT INTO Define_UserRP(IdUser,IdP) VALUES(@IdUser,@IdP)", ConectionBank)
                For i As Integer = 0 To DGV.RowCount - 2
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "رابطه کاربر و طرف حساب", "جدید", " کاربر :" & CmbUser.Text, "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "AddEdit_PTF", "SavePeople")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Me.RefreshBank()
            Return False
    ...
    Private Function EditPeople(ByVal IdUser As Long) As Boolean
        ...
        Try
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "رابطه کاربر و طرف حساب", "ویرایش", If(Tmp_OneGroup = CmbUser.Text.Trim, "ویرایش کاربر :" & CmbUser.Text.Trim, "ویرایش از کاربر :" & Tmp_OneGroup & " به " & CmbUser.Text), "")
        ...
        Catch ex As Exception
            ...
            If Err.Number = 5 Then
                MessageBox.Show("اطلاعات قابل ذخیره شدن نیست لطفا این پنجره را بسته و دوباره امتحان کنید ", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "AddEdit_PTF", "EditPeople")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Me.RefreshBank()
            Return False
    ...
    Private Sub Fill_User()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "AddEdit_PTF", "Fill_User")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            ...
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "AddEdit_PTF", "Button1_Click")
        End Try
    End Sub
