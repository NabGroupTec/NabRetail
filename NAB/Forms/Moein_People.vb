...
Public Class Moein_People
    ...
    Private Sub Moein_People_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Next
        Access_Form = Get_Access_Form("F77")
    ...
    Private Sub SetMoein(ByVal id As Long)
            ...
            If ChkFac.Checked = True Then
                If dt.Rows.Count > 0 Then
                    For r As Integer = 0 To dt.Rows.Count - 1
                        If dt.Rows(r).Item("Type_Sanad") = 0 Then
                            Dim listMon() As DataRow = dt.Select("Type=" & dt.Rows(r).Item("Type") & " AND Number_Type=" & dt.Rows(r).Item("Number_Type") & " AND (Type_Sanad>=1 AND Type_Sanad<=4)")
                            If listMon.Length > 0 Then
                                For j As Integer = 0 To listMon.Length - 1
                                    If dt.Rows(r).Item("BesMon") > 0 Then
                                        If listMon(j).Item("Type_Sanad") = 1 Then dt.Rows(r).Item("BesMon") -= (listMon(j).Item("BesMon") + listMon(j).Item("BedMon"))
                                        If listMon(j).Item("Type_Sanad") = 2 Then dt.Rows(r).Item("BesMon") += (listMon(j).Item("BesMon") + listMon(j).Item("BedMon"))
                                        If listMon(j).Item("Type_Sanad") = 3 Then dt.Rows(r).Item("BesMon") -= (listMon(j).Item("BesMon") + listMon(j).Item("BedMon"))
                                        If listMon(j).Item("Type_Sanad") = 4 Then dt.Rows(r).Item("BesMon") -= (listMon(j).Item("BesMon") + listMon(j).Item("BedMon"))
                                    Else
                                        If listMon(j).Item("Type_Sanad") = 1 Then dt.Rows(r).Item("BedMon") -= (listMon(j).Item("BesMon") + listMon(j).Item("BedMon"))
                                        If listMon(j).Item("Type_Sanad") = 2 Then dt.Rows(r).Item("BedMon") += (listMon(j).Item("BesMon") + listMon(j).Item("BedMon"))
                                        If listMon(j).Item("Type_Sanad") = 3 Then dt.Rows(r).Item("BedMon") -= (listMon(j).Item("BesMon") + listMon(j).Item("BedMon"))
                                        If listMon(j).Item("Type_Sanad") = 4 Then dt.Rows(r).Item("BedMon") -= (listMon(j).Item("BesMon") + listMon(j).Item("BedMon"))
                                    End If
                                Next
                            End If
                        End If
                    ...
                    For r As Integer = dt.Rows.Count - 1 To 0 Step -1
                        If dt.Rows(r).Item("Type_Sanad") >= 1 And dt.Rows(r).Item("Type_Sanad") <= 4 And dt.Rows(r).Item("Type") >= 0 And dt.Rows(r).Item("Type") <= 10 Then dt.Rows(r).Delete()
                    Next
                    dt.AcceptChanges()
                End If
            End If
        ...
        Catch ex As Exception
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Moein_People", "SetMoein")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub GetKey(ByVal e As KeyEventArgs)
        If e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "Rep_Moein.htm")
    ...
    Private Sub RefreshBank()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Moein_People", "RefreshBank")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub BtnMoein_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnMoein.Click
        ...
        End If
        Me.RefreshBank()
        TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "معین طرف حساب", "نمایش معین", "نمایش معین طرف حساب :" & TxtName.Text, "")
    End Sub
    ...
    Private Sub BtnReport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnReport.Click
        ...
        Else
            ...
            If ChkFac.Checked = True Then
                FrmPrint.Str2 = "محاسبه جمع فاکتور به صورت خالص"
            Else
                FrmPrint.Str2 = ""
            End If
            FrmPrint.DtMoeinPeople = dt
        End If
        ...
        TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "معین طرف حساب", "چاپ گزارش", "چاپ معین طرف حساب :" & TxtName.Text, "")
        FrmPrint.CHoose = "MOEINPEOPLE"
        FrmPrint.ShowDialog()
    End Sub
    ...
    Private Sub BtnSendSMS_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSendSMS.Click
        ...
        If String.IsNullOrEmpty(TxtMobile.Text.Trim) Or Not IsNumeric(TxtMobile.Text.Trim) Or TxtMobile.Text.Trim = "0" Then
            MessageBox.Show("شماره گیرنده پیام اشتباه است ", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        End If

        Try
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "معین طرف حساب", "SMS ارسال", "SMS ارسال معین طرف حساب :" & TxtName.Text, "")

            Dim NewFrm As New SeneSMS
            NewFrm.TxtNumber.Text = TxtMobile.Text.Trim
            NewFrm.TxtMessage.Text = "حساب شما " & TxtMoein.Text.Trim & "(" & TxtState.Text & ")" & "میباشد. از طرف : " & GetNamCompany()
            NewFrm.ShowDialog()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Moein_People", "BtnSendSMS_Click")
        End Try
    End Sub
