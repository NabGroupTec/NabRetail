...
Public Class PayMoney
    ...
    Private Function RoolBackSanad(ByVal Idfac As Long) As Boolean
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "RoolBackSanad")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Sub CalCulateMon()
        Try
            LMandeh.Text = CDbl(TxtDiscountC.Text) + CDbl(TxtCash.Text) + CDbl(Txtbank.Text) + CDbl(TxtChk.Text) + CDbl(TxtSellChk.Text)
    ...
    Private Sub BtnHavaleh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnHavaleh.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "BtnHavaleh_Click")
        End Try
    End Sub
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        Try
            ...
            If CDbl(TxtCash.Text) > 0 And CmbBox.Text = "" Then
                MessageBox.Show("صندوقی جهت پرداخت نقدی انتخاب نشده است", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
                CmbBox.Focus()
                Exit Sub
            End If

            If CDbl(LMandeh.Text) <= 0 Then
                MessageBox.Show("هیچ مبلغی جهت پرداخت تعیین نشده است", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "BtnSave_Click")
        End Try
    End Sub
    Private Function Savepay() As Boolean
        ...
        Try
            ...
            If CDbl(TxtCash.Text) > 0 Then
                Using Cmd As New SqlCommand("INSERT Moein_Box (D_date,disc,mon,T,IdBox,IdUser) VALUES(@D_date,@disc,@mon,@T,@IdBox,@IdUser);SELECT @@IDENTITY", ConectionBank)
                    ...
                    Cmd.Parameters.AddWithValue("@Disc", SqlDbType.NVarChar).Value = "پرداخت بابت سند " & LIdFac.Text & If(String.IsNullOrEmpty(TxtDisc_Cash.Text), "", " - " & TxtDisc_Cash.Text.Trim)
            ...
            If CDbl(Txtbank.Text) > 0 Then
                Using Cmd As New SqlCommand("INSERT Moein_Bank (D_date,disc,mon,T,IDBank,IdUser) VALUES(@D_date,@disc,@mon,@T,@IDBank,@IdUser);SELECT @@IDENTITY", ConectionBank)
                    Cmd.Parameters.AddWithValue("@D_date", SqlDbType.NVarChar).Value = TxtDate.ThisText
                    Cmd.Parameters.AddWithValue("@Disc", SqlDbType.NVarChar).Value = "پرداخت بابت سند " & LIdFac.Text & If(String.IsNullOrEmpty(TxtDiscbank.Text), "", " - " & TxtDiscbank.Text.Trim)
            ...
            If CDbl(TxtCash.Text) > 0 Then
                Using Cmd As New SqlCommand("INSERT Moein_People (D_date,disc,mon,T,IdPeople,IdUser,Type,Number_Type,Type_Sanad) VALUES(@D_date,@disc,@mon,@T,@IdPeople,@IdUser,@Type,@Number_Type,@Type_Sanad);SELECT @@IDENTITY", ConectionBank)
                    Cmd.Parameters.AddWithValue("@D_date", SqlDbType.NVarChar).Value = TxtDate.ThisText
                    Cmd.Parameters.AddWithValue("@Disc", SqlDbType.NVarChar).Value = " پرداخت نقدی بابت سند" & LIdFac.Text & If(String.IsNullOrEmpty(TxtSanad.Text.Trim), "", " سند دفتری " & TxtSanad.Text) & If(String.IsNullOrEmpty(TxtDisc_Cash.Text), "", " - " & TxtDisc_Cash.Text.Trim)
                    Cmd.Parameters.AddWithValue("@mon", SqlDbType.BigInt).Value = CDbl(TxtCash.Text)
                    Cmd.Parameters.AddWithValue("@T", SqlDbType.Int).Value = 0
                    Cmd.Parameters.AddWithValue("@IdPeople", SqlDbType.BigInt).Value = CLng(TxtIdName.Text)
                    Cmd.Parameters.AddWithValue("@IdUser", SqlDbType.BigInt).Value = IIf(Id_User = 0, DBNull.Value, Id_User)
            ...
            End Using
            ''''''''''''''''ActiveChk
            If CDbl(TxtChk.Text) > 0 Or CDbl(TxtSellChk.Text) Then
                ...
                If CDbl(TxtSellChk.Text) > 0 Then
                    Using Cmd As New SqlCommand("UPDATE Chk_Get_Pay SET Activ=@Active WHERE (Kind=0 AND Current_Kind=1) AND (Current_Type =12 ) AND (Current_Number_Type = " & LIdFac.Text & ")", ConectionBank)
                        Cmd.Parameters.AddWithValue("@Active", SqlDbType.BigInt).Value = 1
                        Cmd.ExecuteNonQuery()
                    End Using
                End If
                Using Cmd As New SqlCommand("UPDATE Chk_Id SET IdPeople =" & TxtIdName.Text & " WHERE Id IN (SELECT ID FROM  Chk_Get_Pay  WHERE ([Type] =12 ) AND (Number_Type = " & LIdFac.Text & "));UPDATE Chk_Id SET Current_IdPeople  =" & TxtIdName.Text & " WHERE Id IN (SELECT ID FROM  Chk_Get_Pay  WHERE ([Current_Type] =12 ) AND (Current_Number_Type = " & LIdFac.Text & "))", ConectionBank)
                    Cmd.ExecuteNonQuery()
                End Using
            End If
            ''''''''''''''''LimitPay
            If LimitListArray.Length > 0 Then
                Using Cmd As New SqlCommand("INSERT INTO PayLimitKharid (IdSanad,IdFactor,Pay) VALUES(@IdSanad,@IdFactor,@Pay)", ConectionBank)
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "پرداخت به طرف حساب", "جدید", "ثبت سند :" & LIdFac.Text & " طرف حساب:" & TxtName.Text & " جمع مبلغ:" & LMandeh.Text & If(CDbl(TxtDiscountC.Text) > 0, " تخفیف:" & TxtDiscountC.Text, "") & If(CDbl(TxtCash.Text) > 0, " نقد:" & TxtCash.Text, "") & If((CDbl(TxtChk.Text) + CDbl(TxtSellChk.Text)) > 0, " چک:" & FormatNumber(CDbl(TxtChk.Text) + CDbl(TxtSellChk.Text), 0), "") & If(CDbl(Txtbank.Text) > 0, " حواله:" & Txtbank.Text, ""), "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "Savepay")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Function Editpay() As Boolean
        ...
        Try
            ...
            End If
            '''''''''''''''Sanad
            Using Cmd As New SqlCommand("Update Get_Pay_Sanad SET D_date=@D_date,IdName=@IdName,AllDisc=@AllDisc,Sanad=@Sanad,IdUser=@IdUser,IdSanadDiscount=@IdSanadDiscount,DiscDiscount=@DiscDiscount,Discount=@Discount,Cash=@Cash,IdSanadCash=@IdSanadCash,IdBoxMoein=@IdBoxMoein,IdBox=@IdBox,DiscCash=@DiscCash,MonHavaleh=@MonHavaleh,IdBankHavaleh=@IdBankHavaleh,DiscHavaleh=@DiscHavaleh,IdSanadHavaleh=@IdSanadHavaleh,IdBankMoein=@IdBankMoein,MonPayChk=@MonPayChk,MonSellChk=@MonSellChk,IdSanadChk=@IdSanadChk,DiscChk=@DiscChk WHERE Id=@Id", ConectionBank)
                Cmd.Parameters.AddWithValue("@D_date", SqlDbType.NVarChar).Value = TxtDate.ThisText
                Cmd.Parameters.AddWithValue("@IdName", SqlDbType.BigInt).Value = CLng(TxtIdName.Text)
                Cmd.Parameters.AddWithValue("@AllDisc", SqlDbType.NVarChar).Value = TxtDisc.Text.Trim
                Cmd.Parameters.AddWithValue("@Sanad", SqlDbType.NVarChar).Value = TxtSanad.Text.Trim
            ...
            If CDbl(TxtChk.Text) > 0 Or CDbl(TxtSellChk.Text) > 0 Then
                ...
                Using Cmd As New SqlCommand("UPDATE Chk_Id SET IdPeople =" & TxtIdName.Text & " WHERE Id IN (SELECT ID FROM  Chk_Get_Pay  WHERE ([Type] =12 ) AND (Number_Type = " & LIdFac.Text & "));UPDATE Chk_Id SET Current_IdPeople  =" & TxtIdName.Text & " WHERE Id IN (SELECT ID FROM  Chk_Get_Pay  WHERE ([Current_Type] =12 ) AND (Current_Number_Type = " & LIdFac.Text & "));UPDATE Moein_People SET IDPeople =" & TxtIdName.Text & " WHERE T=1 AND ID IN (SELECT PeopleMoein FROM Sanad_ChangeChk  WHERE IdChk IN (SELECT ID FROM  Chk_Get_Pay  WHERE ([Current_Type] =12 ) AND (Current_Number_Type = " & LIdFac.Text & ")))", ConectionBank)
                    Cmd.ExecuteNonQuery()
                End Using
            End If
            ''''''''''''''''LimitPay
            Using Cmd As New SqlCommand("Delete FROM  PayLimitKharid  WHERE IdSanad=" & CLng(LIdFac.Text), ConectionBank)
                Cmd.ExecuteNonQuery()
            End Using
            If LimitListArray.Length > 0 Then
                Using Cmd As New SqlCommand("INSERT INTO PayLimitKharid (IdSanad,IdFactor,Pay) VALUES(@IdSanad,@IdFactor,@Pay)", ConectionBank)
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "پرداخت به طرف حساب", "ویرایش", "ویرایش سند :" & LIdFac.Text & " طرف حساب:" & TxtName.Text & " جمع مبلغ:" & LMandeh.Text & If(CDbl(TxtDiscountC.Text) > 0, " تخفیف:" & TxtDiscountC.Text, "") & If(CDbl(TxtCash.Text) > 0, " نقد:" & TxtCash.Text, "") & If((CDbl(TxtChk.Text) + CDbl(TxtSellChk.Text)) > 0, " چک:" & FormatNumber(CDbl(TxtChk.Text) + CDbl(TxtSellChk.Text), 0), "") & If(CDbl(Txtbank.Text) > 0, " حواله:" & Txtbank.Text, ""), "")
            Return True
        Catch ex As Exception
            sqltransaction.Rollback()
            sqltransaction.Dispose()
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "Editpay")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Sub BtnCancle_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnCancle.Click
        Try
            ...
            Me.Close()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "BtnCancle_Click")
        End Try
    End Sub
    ...
    Private Sub BtnSodor_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSodor.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "BtnSodor_Click")
        End Try
    End Sub
    ...
    Private Function CreateSanad() As Long
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "CreateSanad")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return 0
        End Try
    ...
    Private Sub GetInfo()
        Try
            ...
            '////////////////////////////////////////
            Using Cmd As New SqlCommand("SELECT nam,idname,d_date,discount,idsanaddiscount,cash,idsanadcash,monhavaleh,idbankHavaleh,discHavaleh,MonPayChk,MonSellChk,idsanadChk,idsanadHavaleh,DiscChk,DiscSellChk,DiscDiscount ,discCash,IdBox,IdBoxMoein ,IdBankMoein,AllDisc ,Sanad  FROM (Get_Pay_Sanad INNER JOIN Define_People ON Get_Pay_Sanad.Idname =Define_People.ID  ) WHERE Get_Pay_Sanad.Id =@ID", ConectionBank)
                Cmd.Parameters.AddWithValue("@Id", SqlDbType.BigInt).Value = CLng(LIdFac.Text)
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    ''''''''''''''''''''''''''''''''''''
                    If ConectionBank.State = ConnectionState.Closed Then ConectionBank.Open()
                    Using CmdLimit As New SqlCommand("SELECT IdFactor ,Pay  from PayLimitKharid WHERE IdSanad =" & CLng(LIdFac.Text), ConectionBank)
                    ...
                    ''''''''''''''''''''''''''''''''''''
                    For i As Integer = 0 To LimitListArray.Length - 1
                        LimitListArray(i).Mon = GetBedFac(LimitListArray(i).IdFactor, "K")
                    Next
                Else
                    If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "GetInfo")
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Me.Close()
        End Try
    ...
    Private Sub GetBoxList()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "GetBoxList")
        End Try
    End Sub
    ...
    Private Sub BtnDiscDisC_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDiscDisC.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "BtnDiscDisC_Click")
        End Try
    End Sub
    ...
    Private Sub BtnDiscCash_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDiscCash.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "BtnDiscCash_Click")
        End Try
    End Sub
    ...
    Private Sub BtnDiscHavaleh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDiscHavaleh.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "BtnDiscHavaleh_Click")
        End Try
    End Sub
    ...
    Private Sub BtnDiscChk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDiscChk.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "BtnDiscChk_Click")
        End Try
    End Sub
    ...
    Private Sub BtnBox_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnBox.Click
        Try
            ...
            Me.GetBoxList()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "BtnBox_Click")
        End Try
    End Sub
    ...
    Private Sub BtnRelation_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnRelation.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "BtnRelation_Click")
        End Try
    End Sub
    ...
    Private Sub BtnDisc_SellChk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDisc_SellChk.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "BtnDisc_SellChk_Click")
        End Try
    End Sub
    ...
    Private Sub BtnFroshChk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnFroshChk.Click
        Try
            If String.IsNullOrEmpty(TxtIdName.Text) Or String.IsNullOrEmpty(TxtName.Text) Then
                MessageBox.Show("هیچ طرف حسابی جهت خرج چک مشتری انتخاب نشده است", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If
            Using FrmFrosh As New Frosh_Check
                FrmFrosh.LName.Text = TxtName.Text.Trim
                FrmFrosh.LIdname.Text = TxtIdName.Text
                If String.IsNullOrEmpty(LIdFac.Text) Or LIdFac.Text = "0" Then
                    LIdFac.Text = CreateSanad()
                End If
                FrmFrosh.LFac.Text = LIdFac.Text
                FrmFrosh.LState.Text = 12
                FrmFrosh.ShowDialog()
                If LEdit.Text = "1" Then BtnCancle.Enabled = False
                TxtSellChk.Text = FrmFrosh.TxtallmoneyPay.Text
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "PayMoney", "BtnFroshChk_Click")
        End Try
    End Sub
    ...
    Private Sub CmbBox_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmbBox.SelectedIndexChanged
    ...
    Private Sub MNU_FroshChk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MNU_FroshChk.Click
        TxtDisc_SellChk.Text = TxtDisc.Text.Trim
    End Sub
