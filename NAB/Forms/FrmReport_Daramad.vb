...
Public Class FrmReport_Daramad
    ...
    Private Sub FrmOtherReport_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Next
        Access_Form = Get_Access_Form("F86")
    ...
    Private Sub GetDaramad()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmReport_Daramad", "GetDaramad")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        ...
        If ChkMonAll.Checked = False Then
            FrmPrint.PrintSQl = "SELECT  D_date,NamOne + '-' + Nam + ISNULL('  ' + AllDisc,'') As Namcharge,BedMon=0.0 ,(Cash+ MonHavaleh+ MonPayChk+ Lend) AS BesMon,T=N'بس' ,Mandeh=0.0  FROM  Get_Daramad INNER JOIN Define_Daramad ON Get_Daramad.IdDaramad = Define_Daramad.ID INNER JOIN Define_OneDaramad ON Define_Daramad.IdOne = Define_OneDaramad.Id " & ListCharge & If(String.IsNullOrEmpty(ListCharge), If(String.IsNullOrEmpty(Dat), "", " WHERE " & Dat), If(String.IsNullOrEmpty(Dat), "", " AND " & Dat)) & " ORDER BY D_date"
            FrmPrint.CHoose = "DARAMAD"
        Else
            If RdoTwo.Checked = True Then
                FrmPrint.PrintSQl = "SELECT NamCharge ,ISNULL(SUM(BedMon),0) As BedMon ,ISNULL(SUM(BesMon),0) As BesMon ,T ,ISNULL(SUM(Mandeh),0) As Mandeh  FROM (SELECT  D_date,NamOne + '-' + Nam As Namcharge,BedMon=0.0 ,(Cash+ MonHavaleh+ MonPayChk+ Lend) AS BesMon,T=N'بس' ,Mandeh=0.0  FROM  Get_Daramad INNER JOIN Define_Daramad ON Get_Daramad.IdDaramad = Define_Daramad.ID INNER JOIN Define_OneDaramad ON Define_Daramad.IdOne = Define_OneDaramad.Id " & ListCharge & If(String.IsNullOrEmpty(ListCharge), If(String.IsNullOrEmpty(Dat), "", " WHERE " & Dat), If(String.IsNullOrEmpty(Dat), "", " AND " & Dat)) & ")AS AllCharge GROUP By NamCharge ,T ORDER BY NamCharge"
            Else
                FrmPrint.PrintSQl = "SELECT NamCharge ,ISNULL(SUM(BedMon),0) As BedMon ,ISNULL(SUM(BesMon),0) As BesMon ,T ,ISNULL(SUM(Mandeh),0) As Mandeh  FROM (SELECT  D_date,NamOne As Namcharge,BedMon=0.0 ,(Cash+ MonHavaleh+ MonPayChk+ Lend) AS BesMon,T=N'بس' ,Mandeh=0.0  FROM  Get_Daramad INNER JOIN Define_Daramad ON Get_Daramad.IdDaramad = Define_Daramad.ID INNER JOIN Define_OneDaramad ON Define_Daramad.IdOne = Define_OneDaramad.Id " & ListCharge & If(String.IsNullOrEmpty(ListCharge), If(String.IsNullOrEmpty(Dat), "", " WHERE " & Dat), If(String.IsNullOrEmpty(Dat), "", " AND " & Dat)) & ")AS AllCharge GROUP By NamCharge ,T ORDER BY NamCharge"
            End If
            FrmPrint.CHoose = "SUMDARAMAD"
        End If

        TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "درآمد", "تهیه گزارش", "", "")
    ...
    Private Sub GetKey(ByVal e As KeyEventArgs)
        If e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "Rep_Daramad.htm")
