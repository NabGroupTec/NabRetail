...
Public Class ReportBackSell
    ...
    Private Sub FrmSodNKhales_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Next
        Access_Form = Get_Access_Form("F72")
    ...
    Private Sub GetKala()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "ReportBackSell", "GetKala")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub GetPeople()
        Try
            ...
            DGV2.DataSource = dvP
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "ReportBackSell", "GetPeople")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        ...
        Dim Part As String = ""
        If ChkPart.Checked = True Then
            Part = " AND ListFactorBackSell.Part =" & TxtIdPart.Text & " "
        End If

        Dim anbar As String = ""
        If ChkAnbar.Checked = True Then
            anbar = " AND  KalaFactorBackSell.IdAnbar =" & TxtIdAnbar.Text
        End If
        ...
        If ChkSum.Checked = False Then
            FrmPrint.PrintSQl = "SELECT  KalaFactorBackSell.IdFactor, CAST (KolCount AS NvarChar(max)) As KolCount,CAST (JozCount AS NvarChar(max)) As JozCount,Fe,DarsadMon, Mon,D_date, Define_Kala.Nam As KalaNam, Define_OneGroup.NamOne + '-' + Define_TwoGroup.NamTwo As GroupKala, Define_People.Nam AS NamP ,Define_Anbar.nam  As Anbar FROM  KalaFactorBackSell INNER JOIN ListFactorBackSell ON KalaFactorBackSell.IdFactor = ListFactorBackSell.IdFactor INNER JOIN Define_Kala ON KalaFactorBackSell.IdKala = Define_Kala.Id INNER JOIN Define_OneGroup ON Define_Kala.IdCodeOne = Define_OneGroup.Id INNER JOIN Define_TwoGroup ON Define_Kala.IdCodeTwo = Define_TwoGroup.Id AND Define_OneGroup.Id = Define_TwoGroup.IdOne INNER JOIN Define_People ON ListFactorBackSell.IdName = Define_People.ID INNER JOIN Define_Anbar ON Define_Anbar.ID =KalaFactorBackSell.IdAnbar  WHERE ListFactorBackSell.Activ =1 " & anbar & ListPeople & ListKala & dat & Part & visitor & User & If(Chkdiscount.Checked = False, "", " AND DarsadDiscount>=100 ") & Sort
            FrmPrint.CHoose = "REPORTBACKSELL"
        ElseIf ChkSum.Checked = True Then
            If RdoPeople.Checked = True Then
                FrmPrint.PrintSQl = "SELECT Cast (ISNULL(SUM(kolCount),0) As Nvarchar(max))As KolCount ,Cast (ISNULL(SUM(JozCount ),0)  As Nvarchar(Max)) As JozCount,ISNULL(AVG(Fe),0) As Fe,ISNULL(SUM(DarsadMon),0) As DarsadMon,ISNULL(SUM(Mon),0) As Mon,KalaNam ,GroupKala ,NamP  FROM (SELECT   KolCount,JozCount ,Fe,DarsadMon, Mon,D_date, Define_Kala.Nam As KalaNam, Define_OneGroup.NamOne + '-' + Define_TwoGroup.NamTwo As GroupKala, Define_People.Nam AS NamP FROM  KalaFactorBackSell INNER JOIN ListFactorBackSell ON KalaFactorBackSell.IdFactor = ListFactorBackSell.IdFactor INNER JOIN Define_Kala ON KalaFactorBackSell.IdKala = Define_Kala.Id INNER JOIN Define_OneGroup ON Define_Kala.IdCodeOne = Define_OneGroup.Id INNER JOIN Define_TwoGroup ON Define_Kala.IdCodeTwo = Define_TwoGroup.Id AND Define_OneGroup.Id = Define_TwoGroup.IdOne INNER JOIN Define_People ON ListFactorBackSell.IdName = Define_People.ID WHERE ListFactorBackSell.Activ =1 " & anbar & ListPeople & ListKala & dat & Part & visitor & User & If(Chkdiscount.Checked = False, "", " AND DarsadDiscount>=100 ") & ") As AllKaka GROUP BY NamP ,KalaNam ,GroupKala " & Sort
                FrmPrint.CHoose = "SUMREPORTBACKSELL"
            ElseIf RdoPeopleGroup.Checked = True Then
                ...
                FrmPrint.CHoose = "SUMREPORTBACKSELLPGROUP"
            ElseIf RdoListKala.Checked = True Then
                FrmPrint.PrintSQl = "SELECT Cast (ISNULL(SUM(kolCount),0) As Nvarchar(max))As KolCount ,Cast (ISNULL(SUM(JozCount ),0)  As Nvarchar(Max)) As JozCount,ISNULL(AVG(Fe),0) As Fe,ISNULL(SUM(DarsadMon),0) As DarsadMon,ISNULL(SUM(Mon),0) As Mon,KalaNam ,GroupKala  FROM (SELECT   KolCount,JozCount ,Fe,DarsadMon, Mon,D_date, Define_Kala.Nam As KalaNam, Define_OneGroup.NamOne + '-' + Define_TwoGroup.NamTwo As GroupKala, Define_People.Nam AS NamP FROM  KalaFactorBackSell INNER JOIN ListFactorBackSell ON KalaFactorBackSell.IdFactor = ListFactorBackSell.IdFactor INNER JOIN Define_Kala ON KalaFactorBackSell.IdKala = Define_Kala.Id INNER JOIN Define_OneGroup ON Define_Kala.IdCodeOne = Define_OneGroup.Id INNER JOIN Define_TwoGroup ON Define_Kala.IdCodeTwo = Define_TwoGroup.Id AND Define_OneGroup.Id = Define_TwoGroup.IdOne INNER JOIN Define_People ON ListFactorBackSell.IdName = Define_People.ID WHERE ListFactorBackSell.Activ =1 " & anbar & ListPeople & ListKala & dat & Part & visitor & User & If(Chkdiscount.Checked = False, "", " AND DarsadDiscount>=100 ") & ") As AllKaka GROUP BY KalaNam ,GroupKala " & Sort
                FrmPrint.CHoose = "SUMREPORTBACKSELLKALA"
            ElseIf RdoGroup.Checked = True Then
                FrmPrint.PrintSQl = "SELECT Cast (ISNULL(SUM(kolCount),0) As Nvarchar(max))As KolCount ,Cast (ISNULL(SUM(JozCount ),0)  As Nvarchar(Max)) As JozCount,ISNULL(AVG(Fe),0) As Fe,ISNULL(SUM(DarsadMon),0) As DarsadMon,ISNULL(SUM(Mon),0) As Mon ,GroupKala   FROM (SELECT   KolCount,JozCount ,Fe,DarsadMon, Mon,D_date, Define_Kala.Nam As KalaNam, Define_OneGroup.NamOne + '-' + Define_TwoGroup.NamTwo As GroupKala, Define_People.Nam AS NamP FROM  KalaFactorBackSell INNER JOIN ListFactorBackSell ON KalaFactorBackSell.IdFactor = ListFactorBackSell.IdFactor INNER JOIN Define_Kala ON KalaFactorBackSell.IdKala = Define_Kala.Id INNER JOIN Define_OneGroup ON Define_Kala.IdCodeOne = Define_OneGroup.Id INNER JOIN Define_TwoGroup ON Define_Kala.IdCodeTwo = Define_TwoGroup.Id AND Define_OneGroup.Id = Define_TwoGroup.IdOne INNER JOIN Define_People ON ListFactorBackSell.IdName = Define_People.ID WHERE ListFactorBackSell.Activ =1 " & anbar & ListPeople & ListKala & dat & Part & visitor & User & If(Chkdiscount.Checked = False, "", " AND DarsadDiscount>=100 ") & ") As AllKaka GROUP BY GroupKala " & Sort
                FrmPrint.CHoose = "SUMREPORTBACKSELLGROUP"
            End If
        End If

        TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "برگشت از فروش  کالا", "تهیه گزارش", "", "")
        FrmPrint.ShowDialog()
        Me.Enabled = True
    End Sub
    ...
    Private Sub GetKey(ByVal e As KeyEventArgs)
        If e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "Rep_BackSell.htm")
