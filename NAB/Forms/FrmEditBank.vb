    ...
    Private Sub FillData()
        Try
            ...
            For i As Integer = 0 To Tmp_DtBank.Rows.Count - 1
                DGV.Rows.Add()
                DGV.Item("Cln_Id", DGV.RowCount - 1).Value = Tmp_DtBank.Rows(i).Item("Id")
                DGV.Item("Cln_Nam", DGV.RowCount - 1).Value = Tmp_DtBank.Rows(i).Item("Nam")
                DGV.Item("Cln_number_n", DGV.RowCount - 1).Value = Tmp_DtBank.Rows(i).Item("number_n")
                DGV.Item("Cln_NewMandeh", DGV.RowCount - 1).Value = IIf(Tmp_DtBank.Rows(i).Item("NewMandeh") <> 0, FormatNumber(Tmp_DtBank.Rows(i).Item("NewMandeh"), 0), 0)
            Next
            CalculateMon()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmEditBank", "FillData")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        Try
            If DGV.Rows.Count <= 0 Then
                MessageBox.Show("بانکی برای اصلاحیه وجود ندارد", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If
            ...
            FillData()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmEditBank", "Button2_Click")
            Me.Close()
        End Try
    End Sub
End Class