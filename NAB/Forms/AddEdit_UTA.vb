...
Public Class AddEdit_UTA
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        Try
            ...
            If DGV.RowCount <= 1 Then
                MessageBox.Show("انباری انتخاب نشده است", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            Else
                For i As Integer = 0 To DGV.RowCount - 2
                    ...
                    For j As Integer = 0 To DGV.RowCount - 2
                        ...
                        End If
                        If count > 1 Then
                            MessageBox.Show("نام انبار سطر شماره  " & "{" & i + 1 & "}" & " تكراري است لطفا ان را اصلاح نماييد", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            Exit Sub
                        End If
                    Next j
                Next i
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "AddEdit_UTA", "BtnSave_Click")
            Me.RefreshBank()
        End Try
    End Sub
    Private Function CheckPeopleRepeate(ByVal IdUser As Long) As Boolean
        Try
            ...
            Dim Str As String = "SELECT Id,IdUser,IDA FROM Define_UserRPAnbar"
            Ds.Clear()
            ...
            Dta.Fill(Ds, "Define_UserRPAnbar")
            For i As Integer = 0 To DGV.RowCount - 2
                Dim dr() As DataRow = Ds.Tables("Define_UserRPAnbar").Select("IdUser=" & IdUser & " AND IDA=" & DGV.Item("Cln_IdP", i).Value)
                If dr.Length > 0 Then
                    MessageBox.Show(" انبار سطر شماره  " & "{" & i + 1 & "}" & " قبلا برای این کاربر ثبت شده است", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Return False
                End If
            Next
            Return True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "AddEdit_UTA", "CheckPeopleRepeate")
            Return False
        End Try
    ...
    Private Function SavePeople(ByVal IdUser As Long) As Boolean
        ...
        Try
            ...
            Using CmdSelect As New SqlCommand("SELECT Count(IdUser) FROM Define_UserRAnbar WHERE IdUser=@IdUser", ConectionBank)
                CmdSelect.Parameters.AddWithValue("@IdUser", SqlDbType.BigInt).Value = IdUser
                If CmdSelect.ExecuteScalar() <= 0 Then
                    Using Cmd As New SqlCommand("INSERT INTO Define_UserRAnbar(IdUser) VALUES(@IdUser)", ConectionBank)
            ...
            Using Cmd As New SqlCommand("INSERT INTO Define_UserRPAnbar(IdUser,IdA) VALUES(@IdUser,@IdA)", ConectionBank)
                For i As Integer = 0 To DGV.RowCount - 2
                    Cmd.Parameters.AddWithValue("@IdA", SqlDbType.BigInt).Value = CLng(DGV.Item("Cln_IdP", i).Value)
                    Cmd.Parameters.AddWithValue("@IdUser", SqlDbType.BigInt).Value = IdUser
                    Cmd.ExecuteNonQuery()
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "رابطه کاربر و انبار ", "جدید", " کاربر :" & CmbUser.Text, "")
            Return True
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "AddEdit_UTA", "SavePeople")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Me.RefreshBank()
            Return False
    ...
    Private Function EditPeople(ByVal IdUser As Long) As Boolean
        ...
        Try
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "رابطه کاربر و انبار ", "ویرایش", If(Tmp_OneGroup = CmbUser.Text.Trim, "ویرایش کاربر :" & CmbUser.Text.Trim, "ویرایش از کاربر :" & Tmp_OneGroup & " به " & CmbUser.Text), "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "AddEdit_UTA", "EditPeople")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Me.RefreshBank()
            Return False
    ...
    Private Sub Fill_User()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "AddEdit_UTA", "Fill_User")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub BtnDel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDel.Click
        If MessageBox.Show("آيا براي حذف همه انبارهای موجود مطمئن هستيد؟", "سئوال", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.No Then Exit Sub
        DGV.Rows.Clear()
    End Sub
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            ...
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "AddEdit_UTA", "Button1_Click")
        End Try
    End Sub
