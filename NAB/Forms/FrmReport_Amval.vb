...
Public Class FrmReport_Amval
    ...
    Private Sub FrmOtherReport_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Next
        Access_Form = Get_Access_Form("F87")
    ...
    Private Sub GetAmval()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmReport_Amval", "GetAmval")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        ...
        If ChkMonAll.Checked = False Then
            FrmPrint.PrintSQl = "SELECT  D_date,NamOne + '-' + Nam + ISNULL('  ' + AllDisc,'') As Namcharge,BedMon=CASE Get_Pay_Amval.[State]  WHEN 0 THEN (Cash+ MonHavaleh+ MonPayChk+ MonsellChk+ Lend) ELSE 0.0 END , BesMon=CASE Get_Pay_Amval.[State] WHEN 1 THEN (Cash+ MonHavaleh+ MonPayChk+ MonsellChk+ Lend) ELSE 0.0 END ,T=N'  ' ,Mandeh=0.0  FROM  Get_Pay_Amval  INNER JOIN Define_Amval ON Get_Pay_Amval .IdAmval  = Define_Amval .ID INNER JOIN Define_OneAmval ON Define_Amval .IdOne = Define_OneAmval .Id " & ListCharge & If(String.IsNullOrEmpty(ListCharge), If(String.IsNullOrEmpty(Dat), "", " WHERE " & Dat), If(String.IsNullOrEmpty(Dat), "", " AND " & Dat)) & "UNION ALL SELECT D_date=N'',Namcharge=N'اموال اول دوره ' + nam ,BedMon= AllMoney , BesMon=0,T=N'  ', Mandeh=0.0 FROM Define_Amval " & ListCharge & " ORDER BY D_date"
            FrmPrint.CHoose = "AMVAL"
        Else
            If RdoTwo.Checked = True Then
                FrmPrint.PrintSQl = "SELECT NamCharge ,ISNULL(SUM(BedMon),0) As BedMon ,ISNULL(SUM(BesMon),0) As BesMon ,T ,ISNULL(SUM(Mandeh),0) As Mandeh  FROM (SELECT  D_date,NamOne + '-' + Nam  As Namcharge,BedMon=CASE Get_Pay_Amval.[State]  WHEN 0 THEN (Cash+ MonHavaleh+ MonPayChk+ MonsellChk+ Lend) ELSE 0.0 END , BesMon=CASE Get_Pay_Amval.[State] WHEN 1 THEN (Cash+ MonHavaleh+ MonPayChk+ MonsellChk+ Lend) ELSE 0.0 END ,T=N'  ' ,Mandeh=0.0  FROM  Get_Pay_Amval  INNER JOIN Define_Amval ON Get_Pay_Amval .IdAmval  = Define_Amval .ID INNER JOIN Define_OneAmval  ON Define_Amval .IdOne = Define_OneAmval .Id " & ListCharge & If(String.IsNullOrEmpty(ListCharge), If(String.IsNullOrEmpty(Dat), "", " WHERE " & Dat), If(String.IsNullOrEmpty(Dat), "", " AND " & Dat)) & " UNION ALL SELECT D_date=N'',NamOne + '-' + Nam  As Namcharge ,BedMon=AllMoney, BesMon= 0 ,T=N'  ', Mandeh=0.0 FROM Define_Amval INNER JOIN Define_OneAmval ON Define_Amval .IdOne =Define_OneAmval.Id " & ListCharge & "  )AS AllCharge GROUP By NamCharge ,T ORDER BY NamCharge"
            Else
                FrmPrint.PrintSQl = "SELECT NamCharge ,ISNULL(SUM(BedMon),0) As BedMon ,ISNULL(SUM(BesMon),0) As BesMon ,T ,ISNULL(SUM(Mandeh),0) As Mandeh  FROM (SELECT  D_date,NamOne  As Namcharge,BedMon=CASE Get_Pay_Amval.[State]  WHEN 0 THEN (Cash+ MonHavaleh+ MonPayChk+ MonsellChk+ Lend) ELSE 0.0 END , BesMon=CASE Get_Pay_Amval.[State] WHEN 1 THEN (Cash+ MonHavaleh+ MonPayChk+ MonsellChk+ Lend) ELSE 0.0 END ,T=N'  ' ,Mandeh=0.0  FROM  Get_Pay_Amval  INNER JOIN Define_Amval ON Get_Pay_Amval .IdAmval  = Define_Amval .ID INNER JOIN Define_OneAmval  ON Define_Amval .IdOne = Define_OneAmval .Id " & ListCharge & If(String.IsNullOrEmpty(ListCharge), If(String.IsNullOrEmpty(Dat), "", " WHERE " & Dat), If(String.IsNullOrEmpty(Dat), "", " AND " & Dat)) & " UNION ALL SELECT D_date=N'',NamOne As Namcharge ,BedMon=AllMoney, BesMon= 0 ,T=N'  ', Mandeh=0.0 FROM Define_Amval INNER JOIN Define_OneAmval ON Define_Amval .IdOne =Define_OneAmval.Id " & ListCharge & "  )AS AllCharge GROUP By NamCharge ,T ORDER BY NamCharge"
            End If
            FrmPrint.CHoose = "SUMAMVAL"
        End If

        TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "اموال", "تهیه گزارش", "", "")
    ...
    Private Sub GetKey(ByVal e As KeyEventArgs)
        If e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "Rep_Amval.htm")
