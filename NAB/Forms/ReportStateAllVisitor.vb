...
Public Class ReportStateAllVisitor
    ...
    Private Sub ReportFroshAllUser_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Next
        Access_Form = Get_Access_Form("F142")
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        Try
            ...
            If ChkMasir.Checked = True Then
                If DGV2.RowCount <= 0 Then
                    MessageBox.Show("مسیری وجود ندارد", "پیغام", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Exit Sub
                End If
            End If
            ...
            If ChkMasir.Checked = True Then
                ...
                For i As Integer = 0 To DGV2.RowCount - 1
                    If DGV2.Item("Cln_SelectP", i).Value = True Then
                        TmpPart &= DGV2.Item("Cln_IdM", i).Value & ","
                        CountPeople += 1
                    End If
                Next
                If CountPeople <= 0 Then
                    MessageBox.Show("مسیری انتخاب نشده است", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Exit Sub
                Else
                    TmpPart = TmpPart.Substring(0, TmpPart.Length - 1)
                    TmpPart &= ")"
                End If
            End If
            '////////////////////////////////
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "ReportStateAllVisitor", "BtnOk_Click")
        End Try
    End Sub
    ...
    Private Sub GetPart()
        Try
            ...
            sqlstr = "SELECT Define_Part.Code As Id, Define_Part.NamP As Nam,Define_City.Code ,Define_City.NamCI, Define_Ostan.NamO FROM Define_Ostan INNER JOIN Define_City ON Define_Ostan.Code = Define_City.IdOstan INNER JOIN Define_Part ON Define_Ostan.Code = Define_Part.IdOstan AND Define_City.Code = Define_Part.IdCity ORDER BY Define_Ostan.NamO,Define_City.NamCI"
            DtaP = New SqlDataAdapter(sqlstr, DataSource)
            DtaP.Fill(DsP, "Define_Part")
            DGV2.AutoGenerateColumns = False
            DGV2.DataSource = DsP.Tables("Define_Part")
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "ReportStateAllVisitor", "GetPart")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub GetVisitorList()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "ReportStateAllVisitor", "GetVisitorList")
        End Try
    End Sub
    ...
    Private Sub GetKey(ByVal e As KeyEventArgs)
        If e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "RepActionVizitor.htm")
                ...
                If AllSelectKala.Length > 0 Then
                    For i As Integer = 0 To DGV2.RowCount - 1
                        For j As Integer = 0 To AllSelectKala.Length - 1
                            If AllSelectKala(j).IdKala = DGV2.Item("Cln_Code", i).Value Then DGV2.Item("Cln_SelectP", i).Value = True
                        Next
                    Next
                    Array.Resize(AllSelectKala, 0)
                End If
    ...
    Private Sub BtnSeachCity_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSeachCity.Click
        ...
        If Tmp_Namkala <> "" Then
            If DGV2.RowCount > 1 Then
                For i As Integer = 0 To DGV2.RowCount - 1
                    If DGV2.Item("Cln_Code", i).Value = IdKala Then
    ...
    Private Sub ChkMasir_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChkMasir.CheckedChanged
        If ChkMasir.Checked = True Then
            DGV2.Enabled = True
            ChkAllp.Enabled = True
            BtnSelect.Enabled = True
            BtnSeachCity.Enabled = True
        Else
            ChkAllp.Checked = False
            DGV2.Enabled = False
            ChkAllp.Enabled = False
            BtnSelect.Enabled = False
            BtnSeachCity.Enabled = False
        End If
    End Sub
End Class