...
Public Class City_List
    ...
    Private Sub GetCity()
        Try
            ...
            End If
            '''''''''''''''''''''''''''
            Dta = New SqlDataAdapter("SELECT Define_Ostan.NamO, Define_City.Code AS IdCity, Define_City.NamCI, Define_City.IdOstan FROM Define_Ostan INNER JOIN Define_City ON Define_Ostan.Code = Define_City.IdOstan ORDER BY IdOstan ,Define_City.Code ", DataSource)
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "City_List", "GetCity")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub RefreshBank()
        Try
            ...
            Next
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "City_List", "RefreshBank")
        End Try
    End Sub
    ...
    Private Sub TxtSearch_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtSearch.TextChanged
        Try
            ...
            Else
                If RdoAval.Checked = True Then
                    If RdoAllKala.Checked = True Then
                        dv.RowFilter = "NamCI Like '" & TxtSearch.Text.Trim & "%'"
                    ElseIf RdoOne.Checked = True Then
                        dv.RowFilter = "NamCI Like '" & TxtSearch.Text.Trim & "%'  AND NamO Like '" & CmbOstan.Text.Trim & "%'"
                    End If
                ElseIf RdoAll.Checked = True Then
                    If RdoAllKala.Checked = True Then
                        dv.RowFilter = "NamCI Like '%" & TxtSearch.Text.Trim & "%'"
            ...
            ElseIf e.KeyCode = Keys.Enter Then
                Try
                    If DGV.RowCount > 0 Then
                        IdKala = CLng(DGV.Item("Cln_IdCity", DGV.CurrentRow.Index).Value.ToString)
                        Tmp_Namkala = DGV.Item("Cln_City", DGV.CurrentRow.Index).Value.ToString
                        Tmp_Mon = CLng(DGV.Item("Cln_IdOstan", DGV.CurrentRow.Index).Value.ToString)
                    ...
                    If DGV.RowCount > 0 Then
                        ...
                        For i As Integer = 0 To DGV.RowCount - 1
                            If DGV.Item("Cln_Select", i).Value = True Then
                                ...
                                AllSelectKala(AllSelectKala.Length - 1).Namekala = DGV.Item("Cln_City", i).Value
                                AllSelectKala(AllSelectKala.Length - 1).OneGroup = DGV.Item("Cln_Ostan", i).Value
                                AllSelectKala(AllSelectKala.Length - 1).Id = DGV.Item("Cln_IdOstan", i).Value
                                AllSelectKala(AllSelectKala.Length - 1).IdKala = DGV.Item("Cln_IdCity", i).Value
    ...
    Private Sub CmbOne_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmbOstan.SelectedIndexChanged
        Try
            If RdoAval.Checked = True Then
                dv.RowFilter = "NamO Like '" & CmbOstan.Text.Trim & "%' AND NamCI Like '" & TxtSearch.Text.Trim & "%'"
            ElseIf RdoAll.Checked = True Then
                dv.RowFilter = "NamO Like '%" & CmbOstan.Text.Trim & "%' AND NamCI Like '%" & TxtSearch.Text.Trim & "%'"
            End If
        Catch ex As Exception
    ...
    Private Sub BtnNewP_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnNewP.Click
        Try
            Fnew = True
            Using FrmPeople As New DefineCity
                FrmPeople.ShowDialog()
            End Using
            Fnew = False
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "City_List", "BtnNewP_Click")
        End Try
    End Sub
