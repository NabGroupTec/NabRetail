...
Public Class Moein_Bank
    ...
    Private Sub Moein_People_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Next
        Access_Form = Get_Access_Form("F81")
    ...
    Private Sub SetMoein(ByVal id As Long)
        Try
            ...
            If ChkTime.Checked = False And ChkId.Checked = False Then
                Sql_Str = "SELECT Id , D_date, disc, BedMon=Case When T=0 THEN mon Else 0 END, BesMon=Case When T=1 THEN mon Else 0 END,T=N'  ',Mandeh=0.0,IdUser FROM Moein_Bank WHERE IDBank =" & id & "UNION SELECT id=0,d_date=N'',Disc=N'مانده اول دوره ', BedMon= AllMoney,BesMon=0, T=N'  ',Mandeh=0,IdUser=-1 from Define_Bank WHERE ID =" & id & " ORDER BY D_date ,Id "
            ElseIf ChkTime.Checked = True And ChkId.Checked = False Then
                If ChkAzDate.Checked = True And ChkTaDate.Checked = True Then
                    Sql_Str = "SELECT Id , D_date, disc, BedMon=Case When T=0 THEN mon Else 0 END, BesMon=Case When T=1 THEN mon Else 0 END, T=N'  ',Mandeh=0.0,IdUser FROM Moein_Bank WHERE IDBank = " & id & " AND (D_date>=N'" & FarsiDate1.ThisText & "' AND D_date <=N'" & FarsiDate2.ThisText & "') UNION (SELECT id=0,d_date=N'',Disc=N' مانده اول دوره تا قبل از" & FarsiDate1.ThisText & "', BedMon=Case When AllMoein.Moein>=0 THEN AllMoein.Moein Else 0 END,BesMon=Case When AllMoein.Moein<0 THEN AllMoein.Moein*(-1) Else 0 END,T=N'  ',Mandeh=0.0,IdUser=-1 FROM (SELECT SUM(OnMoney+bed+bes) AS Moein FROM (SELECT ISNULL(SUM(MON),0) AS BED FROM Moein_Bank WHERE IDBank = " & id & " AND T=0 AND D_Date<N'" & FarsiDate1.ThisText & "') AS Bed,(SELECT ISNULL(SUM(MON),0)*(-1) AS BES FROM Moein_Bank WHERE IDBank =" & id & "  AND T=1 AND D_Date<N'" & FarsiDate1.ThisText & "') As Bes,(SELECT ISNULL(SUM(AllOneMoney.Allmoney),0) As OnMoney FROM (SELECT  Allmoney FROM Define_Bank WHERE Id=" & id & " )As AllOneMoney )As One)AllMoein) ORDER BY D_date ,Id"
                ElseIf ChkAzDate.Checked = True And ChkTaDate.Checked = False Then
                    Sql_Str = "SELECT Id , D_date, disc, BedMon=Case When T=0 THEN mon Else 0 END, BesMon=Case When T=1 THEN mon Else 0 END, T=N'  ',Mandeh=0.0,IdUser FROM Moein_Bank WHERE IDBank = " & id & " AND (D_date>=N'" & FarsiDate1.ThisText & "') UNION (SELECT id=0,d_date=N'',Disc=N'مانده اول دوره تا قبل از  " & FarsiDate1.ThisText & "', BedMon=Case When AllMoein.Moein>=0 THEN AllMoein.Moein Else 0 END,BesMon=Case When AllMoein.Moein<0 THEN AllMoein.Moein*(-1) Else 0 END,T=N'  ',Mandeh=0.0,IdUser=-1 FROM (SELECT SUM(OnMoney+bed+bes) AS Moein FROM (SELECT ISNULL(SUM(MON),0) AS BED FROM Moein_Bank WHERE IDBank = " & id & " AND T=0 AND D_Date<N'" & FarsiDate1.ThisText & "') AS Bed,(SELECT ISNULL(SUM(MON),0)*(-1) AS BES FROM Moein_Bank WHERE IDBank =" & id & "  AND T=1 AND D_Date<N'" & FarsiDate1.ThisText & "') As Bes,(SELECT ISNULL(SUM(AllOneMoney.Allmoney),0) As OnMoney FROM (SELECT  Allmoney FROM Define_Bank WHERE Id=" & id & " )As AllOneMoney )As One)AllMoein) ORDER BY D_date ,Id"
                ElseIf ChkAzDate.Checked = False And ChkTaDate.Checked = True Then
                    Sql_Str = "SELECT Id , D_date, disc, BedMon=Case When T=0 THEN mon Else 0 END, BesMon=Case When T=1 THEN mon Else 0 END, T=N'  ',Mandeh=0.0,IdUser FROM Moein_Bank WHERE IDBank = " & id & " AND (D_date<=N'" & FarsiDate2.ThisText & "') UNION SELECT id=0,d_date=N'',Disc=N' مانده اول دوره', BedMon= AllMoney,BesMon=0, T=N'  ',Mandeh=0.0,IdUser=-1 from Define_Bank WHERE ID =" & id & " ORDER BY D_date ,Id"
                End If
            ElseIf ChkTime.Checked = False And ChkId.Checked = True Then
                If ChkAzId.Checked = True And ChkTaId.Checked = True Then
                    Sql_Str = "SELECT Id , D_date, disc, BedMon=Case When T=0 THEN mon Else 0 END, BesMon=Case When T=1 THEN mon Else 0 END, T=N'  ',Mandeh=0.0,IdUser FROM Moein_Bank WHERE IDBank = " & id & " AND (Id>=" & TxtId1.Text & " AND Id <=" & TxtId2.Text & ") UNION (SELECT id=0,d_date=N'',Disc=N' مانده اول دوره تا قبل از سند" & TxtId1.Text & "', BedMon=Case When AllMoein.Moein>=0 THEN AllMoein.Moein Else 0 END,BesMon=Case When AllMoein.Moein<0 THEN AllMoein.Moein*(-1) Else 0 END,T=N'  ',Mandeh=0.0,IdUser=-1 FROM (SELECT SUM(OnMoney+bed+bes) AS Moein FROM (SELECT ISNULL(SUM(MON),0) AS BED FROM Moein_Bank WHERE IDBank = " & id & " AND T=0 AND Id<" & TxtId1.Text & ") AS Bed,(SELECT ISNULL(SUM(MON),0)*(-1) AS BES FROM Moein_Bank WHERE IDBank =" & id & "  AND T=1 AND Id<" & TxtId1.Text & ") As Bes,(SELECT ISNULL(SUM(AllOneMoney.Allmoney),0) As OnMoney FROM (SELECT  Allmoney FROM Define_Bank WHERE Id=" & id & " )As AllOneMoney )As One)AllMoein) ORDER BY D_date ,Id"
                ElseIf ChkAzId.Checked = True And ChkTaId.Checked = False Then
                    Sql_Str = "SELECT Id , D_date, disc, BedMon=Case When T=0 THEN mon Else 0 END, BesMon=Case When T=1 THEN mon Else 0 END, T=N'  ',Mandeh=0.0,IdUser FROM Moein_Bank WHERE IDBank = " & id & " AND (Id>=" & TxtId1.Text & ") UNION (SELECT id=0,d_date=N'',Disc=N'مانده اول دوره تا قبل از سند  " & TxtId1.Text & "', BedMon=Case When AllMoein.Moein>=0 THEN AllMoein.Moein Else 0 END,BesMon=Case When AllMoein.Moein<0 THEN AllMoein.Moein*(-1) Else 0 END,T=N'  ',Mandeh=0.0,IdUser=-1 FROM (SELECT SUM(OnMoney+bed+bes) AS Moein FROM (SELECT ISNULL(SUM(MON),0) AS BED FROM Moein_Bank WHERE IDBank = " & id & " AND T=0 AND Id<" & TxtId1.Text & ") AS Bed,(SELECT ISNULL(SUM(MON),0)*(-1) AS BES FROM Moein_Bank WHERE IDBank =" & id & "  AND T=1 AND Id<" & TxtId1.Text & ") As Bes,(SELECT ISNULL(SUM(AllOneMoney.Allmoney),0) As OnMoney FROM (SELECT  Allmoney FROM Define_Bank WHERE Id=" & id & " )As AllOneMoney )As One)AllMoein) ORDER BY D_date ,Id"
                ElseIf ChkAzId.Checked = False And ChkTaId.Checked = True Then
                    Sql_Str = "SELECT Id , D_date, disc, BedMon=Case When T=0 THEN mon Else 0 END, BesMon=Case When T=1 THEN mon Else 0 END, T=N'  ',Mandeh=0.0,IdUser FROM Moein_Bank WHERE IDBank = " & id & " AND (Id<=" & TxtId2.Text & ") UNION SELECT id=0,d_date=N'',Disc=N' مانده اول دوره', BedMon= AllMoney,BesMon=0, T=N'  ',Mandeh=0.0,IdUser=-1 from Define_Bank WHERE ID =" & id & " ORDER BY D_date ,Id"
                End If
            End If
            dt.Clear()
        ...
        Catch ex As Exception
            If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Moein_Bank", "SetMoein")
            Me.Close()
        End Try
    End Sub
    Private Sub GetKey(ByVal e As KeyEventArgs)
        If e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "Rep_Bank.htm")
    ...
    Private Sub RefreshBank()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Moein_Bank", "RefreshBank")
            Me.Close()
        End Try
    End Sub
    Private Sub BtnMoein_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnMoein.Click
        If String.IsNullOrEmpty(TxtIdName.Text.Trim) Or String.IsNullOrEmpty(TxtName.Text.Trim) Then
            DGV.DataSource = Nothing
            MessageBox.Show("هیچ بانکی انتخاب نشده است", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Me.Enabled = True
            TxtName.Focus()
            Exit Sub
        End If
        ...
        End If
        Me.RefreshBank()
        TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "معین بانک", "نمایش معین", "نمایش معین بانک :" & TxtName.Text, "")
    End Sub
    Private Sub BtnReport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnReport.Click
        ...
        If AllMoein.Length > 0 Then
            ...
            TmpAddress = TxtAccount.Text.Trim
        ...
        Else
            ...
            FrmPrint.DtMoeinBank = dt

        End If

        TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "معین بانک", "چاپ گزارش", "چاپ معین بانک :" & TxtName.Text, "")

        FrmPrint.CHoose = "MOEINBANK"
        FrmPrint.ShowDialog()
    End Sub
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            ...
            End Using
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Moein_bank", "Button1_Click")
        End Try
    End Sub
