...
Public Class Sarmayeh_List
    ...
    Private Sub GetSarmayeh()
        Try
            ...
            End If
            '''''''''''''''''''''''''''
            Dta = New SqlDataAdapter("SELECT  Define_Sarmayeh .nam, Define_Sarmayeh .ID, Define_Sarmayeh .IdCodeTwo, Define_Sarmayeh .IdOne, Define_OneSarmayeh .NamOne FROM  Define_Sarmayeh  INNER JOIN  Define_OneSarmayeh  ON Define_Sarmayeh .IdOne = Define_OneSarmayeh .Id", DataSource)
            Dta.Fill(Ds, "Define_Sarmayeh")
            CmbOstan.Items.Clear()
            For i As Integer = 0 To Ds.Tables("Define_Sarmayeh").Rows.Count - 1
                If CmbOstan.FindStringExact(Ds.Tables("Define_Sarmayeh").Rows(i).Item("NamOne")) = -1 Then
                    CmbOstan.Items.Add(Ds.Tables("Define_Sarmayeh").Rows(i).Item("NamOne"))
                End If
            Next
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Sarmayeh_List", "GetSarmayeh")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub RefreshBank()
        Try
            ...
            Next
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Sarmayeh_List", "RefreshBank")
        End Try
    End Sub
    ...
    Private Sub BtnNewP_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnNewP.Click
        Try
            Fnew = True
            Using FrmPeople As New DefineSarmayeh
                FrmPeople.ShowDialog()
            End Using
            Fnew = False
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "Sarmayeh_List", "BtnNewP_Click")
        End Try
    End Sub
