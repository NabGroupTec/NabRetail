...
Public Class FrmMojodyBank
    ...
    Private Sub FrmMojodyBox_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Next
        Access_Form = Get_Access_Form("F82")
        If (Access_Form <> "-1") Then
            ...
            Else
                If Access_Form.Substring(0, 1) = 0 Or Access_Form.Substring(1, 1) = 0 Or Access_Form.Substring(2, 1) = 0 Then
                    ...
                    Me.Close()
                Else
                    TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "موجودی بانک", "ورود", "", "")
                End If
    ...
    Private Sub Show_AllBank()
        Try
            Dim ds_List As New DataTable
            Dim dta_List As New SqlDataAdapter
            ds_List.Clear()
            If Not dta_List Is Nothing Then dta_List.Dispose()
            dta_List = New SqlDataAdapter("SELECT id,Nam,number_n,mandeh=Case WHEN (MoeinBank +AllMoney)<0 THEN (MoeinBank +AllMoney) * -1 ELSE (MoeinBank +AllMoney) END  ,T=Case WHEN  MoeinBank + AllMoney >=0 THEN N'بد' ELSE N'بس' END  FROM (SELECT ID,n_bank AS Nam,number_n,(SELECT (SUM(bed+bes))AS Moein FROM (SELECT ISNULL(SUM(MON),0) AS BED FROM Moein_Bank WHERE IDBank =Define_Bank .ID AND T=0) AS Bed,(SELECT ISNULL(SUM(MON),0)*(-1) AS BES FROM Moein_Bank WHERE IDBank =Define_Bank .ID AND T=1) As Bes ) AS MoeinBank , ISNULL(Allmoney,0) AS AllMoney FROM Define_Bank) As AllMoeinBank ORDER By mandeh", DataSource)
            dta_List.Fill(ds_List)
            DGV.AutoGenerateColumns = False
            DGV.DataSource = ds_List
            CalculateMon()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "FrmMojodyBank", "Show_AllBank")
        End Try
    End Sub
    ...
    Private Sub GetKey(ByVal e As KeyEventArgs)
        ...
        ElseIf e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "Rep_MojodiBank.htm")
    ...
    Private Sub BtnReport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnReport.Click
        ...
        TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "موجودی بانک", "تهیه گزارش", "", "")

        Using FPrint As New FrmPrint
            Me.Enabled = False
            FPrint.PrintSQl = "SELECT Nam,number_n,mandeh=Case WHEN (MoeinBank +AllMoney)<0 THEN (MoeinBank +AllMoney) * -1 ELSE (MoeinBank +AllMoney) END  ,T=Case WHEN  MoeinBank + AllMoney >=0 THEN N'بد' ELSE N'بس' END  FROM (SELECT ID,n_bank AS Nam,number_n,(SELECT (SUM(bed+bes))AS Moein FROM (SELECT ISNULL(SUM(MON),0) AS BED FROM Moein_Bank WHERE IDBank =Define_Bank .ID AND T=0) AS Bed,(SELECT ISNULL(SUM(MON),0)*(-1) AS BES FROM Moein_Bank WHERE IDBank =Define_Bank .ID AND T=1) As Bes ) AS MoeinBank , ISNULL(Allmoney,0) AS AllMoney FROM Define_Bank) As AllMoeinBank ORDER By mandeh "
            FPrint.CHoose = "MOJODYBANK"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If DGV.RowCount <= 0 Then
            MessageBox.Show("بانکی برای نمایش معین وجود ندارد", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Show_AllBank()
            Exit Sub
        End If
        Using moeinBank As New Moein_Bank
            Me.Enabled = False
            moeinBank.TxtName.Text = DGV.Item("Cln_Nam", DGV.CurrentRow.Index).Value
            moeinBank.TxtIdName.Text = DGV.Item("Cln_Id", DGV.CurrentRow.Index).Value
            moeinBank.ShowDialog()
            Me.Enabled = True
        End Using
    End Sub
