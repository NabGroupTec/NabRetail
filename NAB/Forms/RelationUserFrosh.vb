...
Public Class RelationUserFrosh
    ...
    Private Sub RelationVisitorFrosh_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If EDIT <> 0 Then
            Try
                ...
                Using cmd As New SqlCommand("SELECT REPLACE(ListKalaU.Kala,'.','/') As Kala ,Idkala,ListKalaU.Id, ListKalaU.Mon, Define_Kala.Nam, Define_OneGroup.NamOne + '-' + Define_TwoGroup.NamTwo AS GroupKala FROM ListKalaU INNER JOIN Define_Kala ON ListKalaU.Idkala = Define_Kala.Id INNER JOIN Define_OneGroup ON Define_Kala.IdCodeOne = Define_OneGroup.Id INNER JOIN Define_TwoGroup ON Define_Kala.IdCodeTwo = Define_TwoGroup.Id AND Define_OneGroup.Id = Define_TwoGroup.IdOne WHERE IdLinkUser =" & EDIT, ConectionBank)
                ...
                For i As Integer = 0 To DGV1.RowCount - 2
                    dt.Clear()
                    Using cmd As New SqlCommand("SELECT REPLACE(ListDarsadU.Frosh,'.','/') As Frosh, REPLACE(ListDarsadU.Darsad,'.','/') AS Darsad, Define_Group_P.nam,ListDarsadU.IdGroup FROM ListDarsadU INNER JOIN Define_Group_P ON ListDarsadU.IdGroup = Define_Group_P.Id WHERE IdlinkKala =" & DGV1.Item("Cln_Id", i).Value, ConectionBank)
                ...
                If ConectionBank.State <> ConnectionState.Open Then ConectionBank.Open()
                dtr = Nothing
                Using cmd As New SqlCommand("SELECT  REPLACE(ListOrderDarsadU.Darsad,'.','/') As Darsad, Define_Group_P.nam,IdGroup FROM ListOrderDarsadU INNER JOIN Define_Group_P ON ListOrderDarsadU.IdGroup = Define_Group_P.Id WHERE IdLinkUser =" & EDIT, ConectionBank)
                    dtr = cmd.ExecuteReader
                End Using
                If dtr.HasRows Then
            ...
            Catch ex As Exception
                If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh", "RelationVisitorFrosh_Load")
                Me.Close()
            End Try
        End If
    End Sub
    ...
    Private Sub GetKey(ByVal e As KeyEventArgs)
        If e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "ByKalaUser.htm")
            ...
            Try
                If AllSelectKala.Length > 0 Then
                    ...
                    For i As Integer = 0 To AllSelectKala.Length - 1
                        DGV1.Rows.Add()
                        DGV1.Item("cln_type", DGV1.RowCount - 1).Value = AllSelectKala(i).OneGroup + "-" + AllSelectKala(i).TwoGroup
                        DGV1.Item("Cln_Code", DGV1.RowCount - 1).Value = AllSelectKala(i).IdKala
                        DGV1.Item("Cln_Name", DGV1.RowCount - 1).Value = AllSelectKala(i).Namekala
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        Try
            ...
            Me.Enabled = True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh", "BtnSave_Click")
        End Try
    End Sub
    ...
    Private Function AreYouAdd(ByVal Idvisitior As Long) As Boolean
        Try
            ...
            Using cmd As New SqlCommand("SELECT  COUNT(IdUser) FROM ListUser WHERE IdUser =" & Idvisitior, ConectionBank)
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh", "AreYouAdd")
            Return False
        End Try
    End Function
    ...
    Private Function AreYouEdit(ByVal NewId As Long, ByVal OldId As Long) As Boolean
        Try
            If ConectionBank.State = ConnectionState.Closed Then ConectionBank.Open()
            Using cmd As New SqlCommand("SELECT  COUNT(IdUser) FROM ListUser WHERE IdUser =" & NewId & " AND IdUser <> " & OldId, ConectionBank)
                If cmd.ExecuteScalar <= 0 Then
                    If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
                    Return True
                Else
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh", "AreYouEdit")
            Return False
        End Try
    End Function
    ...
    Private Function SaveList() As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("INSERT INTO ListUser(IdUser) VALUES(@IdUser)", ConectionBank)
                Cmd.Parameters.AddWithValue("@IdUser", SqlDbType.BigInt).Value = CLng(TxtIdVisitor.Text)
                Cmd.ExecuteNonQuery()
            End Using

            Using Cmd As New SqlCommand("INSERT INTO ListKalaU(IdKala,Mon,Kala,IdLinkUser) VALUES(@IdKala,@Mon,@Kala,@IdLinkUser);Select @@IDENTITY", ConectionBank)
                For i As Integer = 0 To DGV1.RowCount - 2
                    ...
                    Using CmdDarsad As New SqlCommand("INSERT INTO ListDarsadU(Frosh,Darsad,IdLinkKala,IdGroup) VALUES(@Frosh,@Darsad,@IdLinkKala,@IdGroup)", ConectionBank)
                        For j As Integer = 0 To DGV2.RowCount - 2
                            If DGV2.Item("Cln_Row", j).Value = i Then
            ...
            Using CmdODarsad As New SqlCommand("INSERT INTO ListOrderDarsadU(IdGroup,Darsad,IdLinkUser) VALUES(@IdGroup,@Darsad,@IdLinkUser)", ConectionBank)
                For j As Integer = 0 To DGV3.RowCount - 2
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), " رابطه کاربر و فروش هدف-کالا", "جدید", "تعیین فروش هدف کاربر :" & TxtVisitor.Text, "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh", "SaveList")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Function EditList() As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("DELETE FROM ListKalaU WHERE IdLinkUser=@IdUser;DELETE FROM  ListOrderDarsadU WHERE IdLinkUser=@IdUser", ConectionBank)
                Cmd.Parameters.AddWithValue("@IdUser", SqlDbType.BigInt).Value = EDIT
                Cmd.ExecuteNonQuery()
            End Using

            Using Cmd As New SqlCommand("Update ListUser SET IdUser=@IdVisit WHERE IdUser=@IdUser", ConectionBank)
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), " رابطه کاربر و فروش هدف-کالا", "ویرایش", If(Tmp_String1 = TxtVisitor.Text, "ویرایش فروش هدف کاربر :" & TxtVisitor.Text, "ویرایش فروش هدف کاربر :" & Tmp_String1 & " به  " & TxtVisitor.Text), "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh", "EditList")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Sub BtnSelect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSelect.Click
        Using frm As New ListRelationUser
            frm.ShowDialog()
            DGV1.AllowUserToAddRows = False
