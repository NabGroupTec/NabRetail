...
Public Class RelationUserFrosh_Two
    ...
    Private Sub RelationVisitorFrosh_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If EDIT <> 0 Then
            Try
                ...
                Using cmd As New SqlCommand("SELECT REPLACE(ListKalaU_G.Kala,'.','/') As Kala ,Idkala,ListKalaU_G.Id, ListKalaU_G.Mon, Define_OneGroup.NamOne AS GroupKala ,Define_TwoGroup.NamTwo As Nam FROM ListKalaU_G  INNER JOIN Define_TwoGroup ON ListKalaU_G.Idkala  = Define_TwoGroup.Id INNER JOIN Define_OneGroup ON Define_TwoGroup .IdOne = Define_OneGroup.Id WHERE IdLinkUser =" & EDIT, ConectionBank)
                ...
                If dtr.HasRows Then
                    ...
                    While dtr.Read
                        DGV1.Rows.Add()
                        DGV1.Item("cln_type", DGV1.RowCount - 1).Value = dtr("GroupKala")
                ...
                For i As Integer = 0 To DGV1.RowCount - 2
                    dt.Clear()
                    Using cmd As New SqlCommand("SELECT REPLACE(ListDarsadU_G.Frosh,'.','/') As Frosh, REPLACE(ListDarsadU_G.Darsad,'.','/') AS Darsad, Define_Group_P.nam,ListDarsadU_G.IdGroup FROM ListDarsadU_G INNER JOIN Define_Group_P ON ListDarsadU_G.IdGroup = Define_Group_P.Id WHERE IdlinkKala =" & DGV1.Item("Cln_Id", i).Value, ConectionBank)
                ...
                If ConectionBank.State <> ConnectionState.Open Then ConectionBank.Open()
                dtr = Nothing
                Using cmd As New SqlCommand("SELECT  REPLACE(ListOrderDarsadU_G.Darsad,'.','/') As Darsad, Define_Group_P.nam,IdGroup FROM ListOrderDarsadU_G INNER JOIN Define_Group_P ON ListOrderDarsadU_G.IdGroup = Define_Group_P.Id WHERE IdLinkUser =" & EDIT, ConectionBank)
                    dtr = cmd.ExecuteReader
                End Using
                If dtr.HasRows Then
            ...
            Catch ex As Exception
                If ConectionBank.State <> ConnectionState.Closed Then ConectionBank.Close()
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh_Two", "RelationVisitorFrosh_Load")
                Me.Close()
            End Try
        End If
    End Sub
    ...
    Private Sub txt_dgv_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txt_dgv.KeyPress
        Try
            ''''''''''''گرفتن نام کالا
            If DGV1.CurrentCell.ColumnIndex = 1 Then
                Dim frmlk As New GroupTwo_List
                frmlk.TxtSearch.Text = e.KeyChar()
                frmlk.ShowDialog()
                DGV1.Focus()
                If Tmp_Namkala <> "" Then
                    DGV1.Item("Cln_type", DGV1.CurrentRow.Index).Value = Tmp_OneGroup
                    DGV1.Item("Cln_name", DGV1.CurrentRow.Index).Value = Tmp_TwoGroup
    ...
    Private Sub GetKey(ByVal e As KeyEventArgs)
        If e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "ByFariKalaUser.htm")
    ...
    Private Sub BtnAdvance_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnAdvance.Click
        Using FrmAdVance As New GroupTwo_List
            ...
            Try
                If AllSelectKala.Length > 0 Then
                    ...
                    For i As Integer = 0 To AllSelectKala.Length - 1
                        DGV1.Rows.Add()
                        DGV1.Item("cln_type", DGV1.RowCount - 1).Value = AllSelectKala(i).OneGroup
                        DGV1.Item("Cln_Code", DGV1.RowCount - 1).Value = AllSelectKala(i).IdKala
                        DGV1.Item("Cln_Name", DGV1.RowCount - 1).Value = AllSelectKala(i).TwoGroup
        ...
        Try
            ...
            If EDIT = 0 Then
                If Not AreYouAdd(CLng(TxtIdVisitor.Text)) Then
                    MessageBox.Show("رابطه برای کاربر مورد نظر قبلا ثبت شده است", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Me.Enabled = True
                    Exit Sub
                End If
            ...
            Me.Enabled = True
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh_Two", "BtnSave_Click")
        End Try
    End Sub
    ...
    Private Function AreYouAdd(ByVal Idvisitior As Long) As Boolean
        Try
            ...
            Using cmd As New SqlCommand("SELECT  COUNT(IdUser) FROM ListUser_G WHERE IdUser =" & Idvisitior, ConectionBank)
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh_Two", "AreYouAdd")
            Return False
        End Try
    End Function
    ...
    Private Function AreYouEdit(ByVal NewId As Long, ByVal OldId As Long) As Boolean
        Try
            If ConectionBank.State = ConnectionState.Closed Then ConectionBank.Open()
            Using cmd As New SqlCommand("SELECT  COUNT(IdUser) FROM ListUser_G WHERE IdUser =" & NewId & " AND IdUser <> " & OldId, ConectionBank)
                If cmd.ExecuteScalar <= 0 Then
                    If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
                    Return True
                Else
        ...
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh_Two", "AreYouEdit")
            Return False
        End Try
    End Function
    ...
    Private Function SaveList() As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("INSERT INTO ListUser_G(IdUser) VALUES(@IdUser)", ConectionBank)
                Cmd.Parameters.AddWithValue("@IdUser", SqlDbType.BigInt).Value = CLng(TxtIdVisitor.Text)
                Cmd.ExecuteNonQuery()
            End Using

            Using Cmd As New SqlCommand("INSERT INTO ListKalaU_G(IdKala,Mon,Kala,IdLinkUser) VALUES(@IdKala,@Mon,@Kala,@IdLinkUser);Select @@IDENTITY", ConectionBank)
                For i As Integer = 0 To DGV1.RowCount - 2
                    ...
                    Cmd.Parameters.AddWithValue("@IdLinkUser", SqlDbType.BigInt).Value = CLng(TxtIdVisitor.Text)
                    id = Cmd.ExecuteScalar
                    Cmd.Parameters.Clear()
                    Using CmdDarsad As New SqlCommand("INSERT INTO ListDarsadU_G(Frosh,Darsad,IdLinkKala,IdGroup) VALUES(@Frosh,@Darsad,@IdLinkKala,@IdGroup)", ConectionBank)
                        For j As Integer = 0 To DGV2.RowCount - 2
                            If DGV2.Item("Cln_Row", j).Value = i Then
            ...
            Using CmdODarsad As New SqlCommand("INSERT INTO ListOrderDarsadU_G(IdGroup,Darsad,IdLinkUser) VALUES(@IdGroup,@Darsad,@IdLinkUser)", ConectionBank)
                For j As Integer = 0 To DGV3.RowCount - 2
                    ...
                    CmdODarsad.Parameters.AddWithValue("@IdLinkUser", SqlDbType.BigInt).Value = CLng(TxtIdVisitor.Text)
            ...
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "رابطه کاربر و فروش هدف-گروه فرعی", "جدید", "تعیین فروش هدف کاربر :" & TxtVisitor.Text, "")
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh_Two", "SaveList")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Function EditList() As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("DELETE FROM ListKalaU_G WHERE IdLinkUser=@IdUser;DELETE FROM  ListOrderDarsadU_G WHERE IdLinkUser=@IdUser", ConectionBank)
                Cmd.Parameters.AddWithValue("@IdUser", SqlDbType.BigInt).Value = EDIT
                Cmd.ExecuteNonQuery()
            End Using

            Using Cmd As New SqlCommand("Update ListUser_G SET IdUser=@IdVisit WHERE IdUser=@IdUser", ConectionBank)
            ...
            sqltransaction.Dispose()
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), " رابطه کاربر و فروش هدف-گروه فرعی", "ویرایش", If(Tmp_String1 = TxtVisitor.Text, "ویرایش فروش هدف کاربر :" & TxtVisitor.Text, "ویرایش فروش هدف کاربر :" & Tmp_String1 & " به  " & TxtVisitor.Text), "")
            Return True
        Catch ex As Exception
            sqltransaction.Rollback()
            sqltransaction.Dispose()
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationUserFrosh_Two", "EditList")
            End If
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            Return False
        End Try
    ...
    Private Sub BtnSelect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSelect.Click
        Using frm As New ListRelationUser_Two
            frm.ShowDialog()
            DGV1.AllowUserToAddRows = False
            For i As Integer = 0 To ListKala.Length - 1
                DGV1.Rows.Add()
                DGV1.Item("cln_type", DGV1.RowCount - 1).Value = ListKala(i).GroupKala
