...
Public Class ReportDamage
    ...
    Private Sub FrmSodNKhales_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Next
        Access_Form = Get_Access_Form("F73")
    ...
    Private Sub GetKala()
        ...
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "ReportDamage", "GetKala")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub BtnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnOk.Click
        ...
        Dim anbar As String = ""
        If ChkAnbar.Checked = True Then
            anbar = " AND  KalaFactorDamage.IdAnbar =" & TxtIdAnbar.Text
        End If
        ...
        If ChkSum.Checked = False Then
            FrmPrint.PrintSQl = "SELECT  KalaFactorDamage.IdFactor, CAST (KolCount AS NvarChar(max)) As KolCount,CAST (JozCount AS NvarChar(max)) As JozCount,Fe, Mon,D_date, Define_Kala.Nam As KalaNam, Define_OneGroup.NamOne + '-' + Define_TwoGroup.NamTwo As GroupKala ,Define_Anbar.nam  As Anbar FROM  KalaFactorDamage INNER JOIN ListFactorDamage ON KalaFactorDamage.IdFactor = ListFactorDamage.IdFactor INNER JOIN Define_Kala ON KalaFactorDamage.IdKala = Define_Kala.Id INNER JOIN Define_OneGroup ON Define_Kala.IdCodeOne = Define_OneGroup.Id INNER JOIN Define_TwoGroup ON Define_Kala.IdCodeTwo = Define_TwoGroup.Id AND Define_OneGroup.Id = Define_TwoGroup.IdOne INNER JOIN Define_Anbar ON Define_Anbar.ID =KalaFactorDamage.IdAnbar WHERE ListFactorDamage.Activ =1 " & anbar & ListKala & dat & Part & Sort
            FrmPrint.CHoose = "REPORTDAMAGE"
        ElseIf ChkSum.Checked = True Then
            FrmPrint.PrintSQl = "SELECT Cast (ISNULL(SUM(kolCount),0) As Nvarchar(max))As KolCount ,Cast (ISNULL(SUM(JozCount ),0)  As Nvarchar(Max)) As JozCount,ISNULL(AVG(Fe),0) As Fe,ISNULL(SUM(Mon),0) As Mon,KalaNam ,GroupKala  FROM (SELECT   KolCount,JozCount ,Fe, Mon,D_date, Define_Kala.Nam As KalaNam, Define_OneGroup.NamOne + '-' + Define_TwoGroup.NamTwo As GroupKala FROM  KalaFactorDamage INNER JOIN ListFactorDamage ON KalaFactorDamage.IdFactor = ListFactorDamage.IdFactor INNER JOIN Define_Kala ON KalaFactorDamage.IdKala = Define_Kala.Id INNER JOIN Define_OneGroup ON Define_Kala.IdCodeOne = Define_OneGroup.Id INNER JOIN Define_TwoGroup ON Define_Kala.IdCodeTwo = Define_TwoGroup.Id AND Define_OneGroup.Id = Define_TwoGroup.IdOne  WHERE ListFactorDamage.Activ =1 " & anbar & ListKala & dat & Part & ") As AllKaka GROUP BY KalaNam ,GroupKala " & Sort
            FrmPrint.CHoose = "SUMREPORTDAMAGE"
        End If

        TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "ضایعات کالا", "تهیه گزارش", "", "")
    ...
    Private Sub GetKey(ByVal e As KeyEventArgs)
        If e.KeyCode = Keys.F1 Then
            Help.ShowHelp(Me, Application.StartupPath & "\Help.chm", HelpNavigator.Topic, "Zayeat.htm")
