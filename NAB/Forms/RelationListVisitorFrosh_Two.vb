...
Public Class RelationListVisitorFrosh_Two

    Dim ds As New DataSet
    Dim str As String = "SELECT  ListVisitor_G.IdVisitor, Define_Visitor.Nam FROM ListVisitor_G INNER JOIN Define_Visitor ON ListVisitor_G.IdVisitor = Define_Visitor.Id"
    ...
    Private Sub Fill_KDgv(ByVal Idvisit As Long)
        Try
            DGV1.DataSource = Nothing
            dsK.Clear()
            strK = "SELECT REPLACE(Listkala_G.Kala,'.','/') As Kala , Listkala_G.Mon,Listkala_G.IdKala,Listkala_G.Id,Define_TwoGroup .NamTwo As nam FROM Listkala_G INNER JOIN Define_TwoGroup  ON Listkala_G.Idkala = Define_TwoGroup.Id WHERE IdLinkVisitor =" & Idvisit
            If Not dtaK Is Nothing Then
                dtaK.Dispose()
            End If
            '''''''''''''''''''''''''''
            dtaK = New SqlDataAdapter(strK, DataSource)
            dtaK.Fill(dsK, "Listkala_G")
            DGV1.AutoGenerateColumns = False
            bsK.DataSource = dsK
            bsK.DataMember = "Listkala_G"
            DGV1.DataSource = bsK
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_Two", "fill_Kdgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Fill_DDgv(ByVal Idvisit As Long)
        Try
            DGV2.DataSource = Nothing
            dsD.Clear()
            strD = "SELECT REPLACE(ListDarsad_G.Frosh,'.','/') As Frosh, REPLACE(ListDarsad_G.Darsad,'.','/') AS Darsad, Define_Group_P.nam FROM ListDarsad_G INNER JOIN Define_Group_P ON ListDarsad_G.IdGroup = Define_Group_P.Id WHERE IdlinkKala =" & Idvisit
            If Not dtaD Is Nothing Then
                dtaD.Dispose()
            End If
            '''''''''''''''''''''''''''
            dtaD = New SqlDataAdapter(strD, DataSource)
            dtaD.Fill(dsD, "ListDarsad_G")
            DGV2.AutoGenerateColumns = False
            bsD.DataSource = dsD
            bsD.DataMember = "ListDarsad_G"
            DGV2.DataSource = bsD
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_Two", "fill_Ddgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub Fill_ODgv(ByVal Idvisit As Long)
        Try
            DGV4.DataSource = Nothing
            dsO.Clear()
            strO = "SELECT  REPLACE(ListOrderDarsad_G.Darsad,'.','/') As Darsad, Define_Group_P.nam FROM ListOrderDarsad_G INNER JOIN Define_Group_P ON ListOrderDarsad_G.IdGroup = Define_Group_P.Id WHERE IdLinkVisitor =" & Idvisit
            If Not dtaO Is Nothing Then
                dtaO.Dispose()
            End If
            '''''''''''''''''''''''''''
            dtaO = New SqlDataAdapter(strO, DataSource)
            dtaO.Fill(dsO, "ListOrderDarsad_G")
            DGV4.AutoGenerateColumns = False
            bsO.DataSource = dsO
            bsO.DataMember = "ListOrderDarsad_G"
            DGV4.DataSource = bsO
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_Two", "Fill_ODgv")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub SetButton()
        Try
            ...
            End If
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_Two", "SetButton")
            Me.Close()
        End Try
    End Sub
    Private Sub Fill_Dgv()
        Try
            ...
            dta.Fill(ds, "ListVisitor_G")
            DGV3.AutoGenerateColumns = False
            bs.DataSource = ds
            bs.DataMember = "ListVisitor_G"
            DGV3.DataSource = bs
        Catch ex As Exception
            If ConectionBank.State = ConnectionState.Open Then ConectionBank.Close()
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_Two", "fill_dgv")
            Me.Close()
        End Try
    End Sub
    Private Sub RefreshBank()
        Try
            ...
            dta.Fill(ds, "ListVisitor_G")
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_Two", "RefreshBank")
            Me.Close()
        End Try
    End Sub
    ...
    Private Sub cmdadd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdadd.Click
        Try
            Using FrmAdd As New RelationVisitorFrosh_Two
                FrmAdd.EDIT = 0
                FrmAdd.ShowDialog()
            End Using
            Me.RefreshBank()
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_Two", "cmdadd_Click")
            Me.RefreshBank()
        End Try
    End Sub
    ...
    Private Sub cmddel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmddel.Click
        Try
            ...
            If str = 6 Then
                ...
                Me.RefreshBank()
                TraceUser(Id_User, GetDate(), CStr(Date.Now.ToLongTimeString), "رابطه ویزیتور و فروش هدف-گروه فرعی", "حذف", "حذف فروش هدف ویزیتور :" & nam, "")
            End If
        Catch ex As Exception
            GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_Two", "cmddel_Click")
            Me.RefreshBank()
        End Try
    End Sub
    Private Function DeleteVisit(ByVal Id As Long) As Boolean
        ...
        Try
            ...
            Using Cmd As New SqlCommand("DELETE FROM ListVisitor_G WHERE IdVisitor=@IdVisitor", ConectionBank)
        ...
        Catch ex As Exception
            ...
            Else
                GetError(ex.Message & vbCrLf & vbCrLf & ex.StackTrace, "RelationListVisitorFrosh_Two", "DeleteVisit")
            End If
            Me.RefreshBank()
            Return False
        End Try
